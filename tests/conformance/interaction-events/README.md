...
- **`input/`** — the blocking `input <prompt>` reporter (issue #681, slice I2), the profile's other
  ...
  (`spec/interaction-events.md:174-185`). Per the maintainer's ruling on #657, `input` is tested by
  ...
  **The prompt's `ol-type` (`:179`) is now fixtured — the #768 ruling settled it.** #681 withheld
  ...
  rather than #681's one-off `expected: "text"`. `spec/interaction-events.md:177`/`:179` now state
  ...
  The **blocking** property (`:156-159`) is observable here only as the *pair*
...
- **`when/`** — the `when <event-word> <block>` named event handler (issue #682, slice I3):
  ...
  *firing* is itself one charged instruction, so in `when-firing-counts-against-budget` an empty-bodied
  ...
  (`spec/interaction-events.md:206-211` — a handler runs each time its event occurs, once per
...
- **`every/`** — the `every <n> <block>` repeated timed handler (issue #683, slice I4):
  ...
  **after registration** (not at a global multiple of `n`) and repeats every `n` ticks while a `wait`
  ...
  pins the third rule (`spec/interaction-events.md:231-235`) — a handler delayed by a one-time block
...
- **`dispatch/`** — the deterministic same-tick dispatch order + cancellation (issue #686, slice I7):
  ...
  each in registration order (`spec/interaction-events.md:132-137`) — `cross-kind-order-during-wait`
  ...
  (`spec/interaction-events.md:139-141` requires preserving pending key/click state). Like the harness's
...
- **Intra-kind same-tick delivery order.** `spec/interaction-events.md:132-137` is a four-item MUST —
...
be displayed as learner text" (`spec/interaction-events.md:179`) as defensible, and a fixture is
normative for every implementation. #768 has since been ruled — the prompt MUST be a `word` — and the
spec states that outright at `:177`/`:179`, so the four `input-prompt-*` fixtures described under
...
1. **`when` is persistent** — its block runs each time its event occurs, once per occurrence
   (`spec/interaction-events.md:206-211`).
2. **Coalescing one missed `every` occurrence is required**, not permitted (`:237-244`). The runtime's
   contrary reading — that zero overlapping invocations satisfies an "at most one" *upper bound* —
   was rejected.
3. **`every n` is fixed rate** (`:231-235`), a third rule the audit had not surfaced: the interval
   clock keeps its own schedule and a late invocation does not re-measure the period.
4. **A handler does not extend the run's lifetime** (`:246-252`), a fourth the ruling added while
...
(`spec/interaction-events.md:203-204`): both standard v0.1 words are inherently once-per-run, so with
