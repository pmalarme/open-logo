...
- **`compareMessages` (optional, default `false`)** is the per-fixture opt-in that makes an expected
  ...
  **Opt in only where the spec fixes the words.** The default is what `spec/error-model.md:256-261`
  asks for — "diagnostic identity is `code` plus `params`; prose is presentation" — and `:263-265`
