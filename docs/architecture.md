...
| **Token classes / syntax highlighting** | `parser/src/highlight.ts` | parser | studio editor, docs, external editors | The 15 normative token classes from `spec/tooling.md`, classified from the **grammar** (not regex); case-insensitive keywords; grammatical position decides the class. Takes the **active conformance profile set** (`HighlightOptions.profiles`, the same vocabulary `check()` uses, defaulting to Core Language alone), because `spec/tooling.md#normative-token-class-model` makes the profile block-heads and `tell` `keyword` while their profile is active and `primitive` while it is not; a host MUST pass the set it is really running (the studio passes `STUDIO_PROFILES`) or its output will misrepresent the host's active profiles. Owner language-designer; renderer learner-experience. See `language-designer/syntax-highlighting`. |
