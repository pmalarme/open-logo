...
- **It does not reach a name a program only *declares*.** `global count = 0` and `local count` write
  their names bare; those tokens introduce a binding rather than resolving one, so they carry
  nothing. The same *reason* excludes a procedure's `:param` binding site, which is likewise not a
  use — though it still carries `false`, because the field is total on `:variable`; measured, a bare
  declared name carries no field at all while `:param` carries `false`. What the modifier *does*
  reach is every **use** of a variable name, across all three assignment spellings
