...
at all). **Declaring the class as new, first-class data was never attempted.** `spec/grammar.md#keywords-primitives-and-built-in-names`
...
1. **Two axes, explicitly independent.** `category` answers *may a program declare this name?*
   ...
   (`spec/grammar.md#keywords-primitives-and-built-in-names`), while `tokenClass` is the class *while the entry's own profile is
   active*.
...
2. **The class is measured, not asserted.** The gate re-paints every name through `highlight()` in
   ...
   admits a keyword as an ordinary name (`spec/grammar.md#keywords-primitives-and-built-in-names`), so the manifest's
...
3. **The profile rule is checked against the profile the entry names.** Every subset of the
   ...
   (`spec/tooling.md#normative-token-class-model`); every other name must be unmoved by any of them, and every probe must
...
7. **The row's data-bearing sentences are generated from the declaration.** `spec/tooling.md#normative-token-class-model` no
...
- `spec/tooling.md#normative-token-class-model` stays a single line, so the **39 pre-existing citations** pointing at it do
...
- This does not make the token class *derivable* — it makes it *declared*. `spec/grammar.md#keywords-primitives-and-built-in-names`'s
