...
- `createInputPromptController()` (`src/input-prompt.ts`) — the headless prompt. It is an
  ...
  (`spec/interaction-events.md:156-157`). `dismiss()` is the third path: Stop/Reset withdraw a
...
`spec/interaction-events.md:154-156` explicitly permits continuing to render
...
- `deliverKey(keyWord)` — one key press, as the lowercase word
  `spec/interaction-events.md:267-271` defines.
...
- the program actually registered a handler of that kind, according to its own `primitive` trace
  event (`spec/interaction-events.md:166-168`), so a non-interactive program is never re-executed by
  a stray keystroke;
- **no `input` question is outstanding right now.** `spec/interaction-events.md:154-157` blocks
...
| `lastAnsweredReadTick + 1` | `spec/interaction-events.md:154-157`: a delivery must not land at or before a read it should have followed | yes, 1 test |
...
notification `spec/interaction-events.md:198-202` requires was lost with no diagnostic. One function
...
after. That matters for `spec/interaction-events.md:162-164` — "hold itself open with a long `wait`
...
- **A program is responsive for as many inputs as the learner gives it.** The old counter capped the
  ...
  **Delivery closes for a program whose clock offers no further yield (#1039).** An earlier version
  of this bullet argued from `spec/interaction-events.md:427-430` that *cancellation* is what "stops future
  handler delivery" and nothing names tick exhaustion. Review rejected that reading, correctly:
  `:427-430` says cancellation stops delivery, it does **not** say cancellation is the only way a run
  closes — and `:244-246` says plainly that once the main line has finished "the run closes". The
  ...
  **"Ended" here means the clock offers no further yield**, which is narrower than `:244-250`'s "the
...
under a deferred one) and dropped the key, where `:137-139` requires the most recent key and click state
...
termination" (`:198-202`), so `stop()` schedules it as a named event and replays once before latching
...
`spec/interaction-events.md:270-271` asks implementations to document theirs.
...
(`:260-261`). `src/canvas-interaction.ts` wires both, and neither is a fallback for the other:
...
shortcut either — OpenLogo v0.1 "does not standardize click coordinate reporters" (`:262-264`), which
