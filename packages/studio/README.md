...
- `createInputPromptController()` (`src/input-prompt.ts`) — the headless prompt. It is an
  ...
  (`spec/interaction-events.md#trace-stream-integration`). `dismiss()` is the third path: Stop/Reset withdraw a
...
`spec/interaction-events.md#trace-stream-integration` explicitly permits continuing to render
...
- `deliverKey(keyWord)` — one key press, as the lowercase word
  `spec/interaction-events.md#on_key-key-word-block` defines.
...
- the program actually registered a handler of that kind, according to its own `primitive` trace
  event (`spec/interaction-events.md#trace-stream-integration`), so a non-interactive program is never re-executed by
  a stray keystroke;
- **no `input` question is outstanding right now.** `spec/interaction-events.md#trace-stream-integration` blocks
...
| `lastAnsweredReadTick + 1` | `spec/interaction-events.md#trace-stream-integration`: a delivery must not land at or before a read it should have followed | yes, 1 test |
...
notification `spec/interaction-events.md#when-event-word-block` requires was lost with no diagnostic. One function
...
after. That matters for `spec/interaction-events.md#trace-stream-integration` — "hold itself open with a long `wait`
...
- **A program is responsive for as many inputs as the learner gives it.** The old counter capped the
  ...
  **Delivery closes for a program whose clock offers no further yield (#1039).** An earlier version
  of this bullet argued from `spec/interaction-events.md#errors-and-cancellation` that *cancellation* is what "stops future
  handler delivery" and nothing names tick exhaustion. Review rejected that reading, correctly:
  `spec/interaction-events.md#errors-and-cancellation` says cancellation stops delivery, it does **not** say cancellation is the only way a run
  closes — and `spec/interaction-events.md#every-n-block` says plainly that once the main line has finished "the run closes". The
  ...
  **"Ended" here means the clock offers no further yield**, which is narrower than `spec/interaction-events.md#every-n-block`'s "the
...
- **Known limitation — under the synchronous replay host a handler registered *by* a handler cannot
  ...
  `spec/interaction-events.md#time-ticks-and-handlers` is why there is no later tick to use: *"a handler does not extend
...
under a deferred one) and dropped the key, where `spec/interaction-events.md#time-ticks-and-handlers` requires the most recent key and click state
...
termination" (`spec/interaction-events.md#when-event-word-block`), so `stop()` schedules it as a named event and replays once before latching
...
`spec/interaction-events.md#on_key-key-word-block` asks implementations to document theirs.
...
- **Non-visual turtle state (#229, extended in #410 to include the current source instruction,
  ...
  **#778 — what the region text carries.** A span covering a *block* used to put the whole block
  ...
  `spec/rendering.md#non-visual-state-descriptions`'s worked example included, and nothing here touches the turtle state, the
