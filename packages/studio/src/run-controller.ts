 ...
 * `spec/interaction-events.md:165-168` requires that no OpenLogo instruction and no handler block
 ...
 * exactly there, which is what `:165-168` asks a host to show.
 *
 ...
 * (`spec/interaction-events.md:209-213`: `"stop"` is "a requested stop notification **before**
 ...
 *   whose clock offers no further yield** (`spec/interaction-events.md:255-261`), which #1039's
 ...
 * - **No `input` question is outstanding right now.** `spec/interaction-events.md:165-168` forbids
 ...
 * host and one under a deferred one) and dropped the key outright, where `:148-150` requires the most
...
export interface RunController {
   ...
   * `spec/interaction-events.md:278-282` defines — `"left"`, `"space"`, `"a"`, and so on. Browser
   ...
   * **or** "an equivalent accessible action" (`spec/interaction-events.md:298-299`), which is why
 ...
 * registered" (`spec/interaction-events.md:177-179`), so the run's own trace stream is the record —
...
export function createRunController(
  ...
  // *within* it: `spec/interaction-events.md:165-168` blocks handlers only until the read finishes,
  ...
  // `spec/interaction-events.md:148-150` requires the most recent key/click state to be preserved.
  ...
  function presentPendingRead(read: PendingRead): void {
    ...
    read.host.present({ prompt: read.prompt }, (answer) => {
      ...
      if (answer === undefined) {
        ...
        // one other ending `spec/interaction-events.md:167-168` allows. Publish the cancellation
  ...
  function beginAttempt(
    ...
    executionHost.execute(request, (settlement) => {
      ...
      if (stopNotificationAttempt === attemptId) {
        ...
        // `spec/interaction-events.md:167-168` allows. Withdrawn **here**, keyed to this exact
  ...
  function finishAttempt(
    ...
    // `nextStepEndIndex()` rather than charging the step that just finished: `:173-175`'s case —
   ...
   * `spec/interaction-events.md:255-261` recognises as holding the run open — so it is ended.
   *
   ...
   * `spec/interaction-events.md:255-261`'s "the run closes when the main line has finished", and a
  ...
  function programIsStillRunning(): boolean {
    if (attemptPending) {
      ...
      // where `spec/interaction-events.md:148-150` requires the most recent key/click state to be
   ...
   * it — no question may be outstanding (`spec/interaction-events.md:165-168` forbids running a
   ...
   * refused delivery for the rest of its life, which is stricter than `:165-168` — the spec blocks
   ...
   * `pendingRead === null` is what enforces `:165-168` itself, and it is transient exactly as the
   * spec's "until" is.
   */
   ...
   * (`spec/interaction-events.md:209-213`) rather than input arriving at a live program, and the
  ...
  function stop(): void {
    ...
    // #952 — `spec/interaction-events.md:209-213` makes `"stop"` "a requested stop notification
    ...
    // `:165-168` forbids running a handler block for a read that ended unanswered.
