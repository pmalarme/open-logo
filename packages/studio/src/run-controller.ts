 ...
 * `spec/interaction-events.md:154-157` requires that no OpenLogo instruction and no handler block
 ...
 * exactly there, which is what `:154-157` asks a host to show.
 *
 ...
 * (`spec/interaction-events.md:198-202`: `"stop"` is "a requested stop notification **before**
 ...
 *   whose clock offers no further yield** (`spec/interaction-events.md:244-250`), which #1039's
 ...
 * - **No `input` question is outstanding right now.** `spec/interaction-events.md:154-157` forbids
 ...
 * host and one under a deferred one) and dropped the key outright, where `:137-139` requires the most
...
export interface RunController {
   ...
   * `spec/interaction-events.md:267-271` defines — `"left"`, `"space"`, `"a"`, and so on. Browser
   ...
   * **or** "an equivalent accessible action" (`spec/interaction-events.md:287-288`), which is why
 ...
 * registered" (`spec/interaction-events.md:166-168`), so the run's own trace stream is the record —
...
export function createRunController(
  ...
  // *within* it: `spec/interaction-events.md:154-157` blocks handlers only until the read finishes,
  ...
  // `spec/interaction-events.md:137-139` requires the most recent key/click state to be preserved.
  ...
  function presentPendingRead(read: PendingRead): void {
    ...
    read.host.present({ prompt: read.prompt }, (answer) => {
      ...
      if (answer === undefined) {
        ...
        // one other ending `spec/interaction-events.md:156-157` allows. Publish the cancellation
  ...
  function beginAttempt(
    ...
    executionHost.execute(request, (settlement) => {
      ...
      if (stopNotificationAttempt === attemptId) {
        ...
        // `spec/interaction-events.md:156-157` allows. Withdrawn **here**, keyed to this exact
  ...
  function finishAttempt(
    ...
    // `nextStepEndIndex()` rather than charging the step that just finished: `:162-164`'s case —
   ...
   * `spec/interaction-events.md:244-250` recognises as holding the run open — so it is ended.
   *
   ...
   * `spec/interaction-events.md:244-250`'s "the run closes when the main line has finished", and a
  ...
  function programIsStillRunning(): boolean {
    if (attemptPending) {
      ...
      // where `spec/interaction-events.md:137-139` requires the most recent key/click state to be
   ...
   * it — no question may be outstanding (`spec/interaction-events.md:154-157` forbids running a
   ...
   * refused delivery for the rest of its life, which is stricter than `:154-157` — the spec blocks
   ...
   * `pendingRead === null` is what enforces `:154-157` itself, and it is transient exactly as the
   * spec's "until" is.
   */
   ...
   * (`spec/interaction-events.md:198-202`) rather than input arriving at a live program, and the
  ...
  function stop(): void {
    ...
    // #952 — `spec/interaction-events.md:198-202` makes `"stop"` "a requested stop notification
    ...
    // `:154-157` forbids running a handler block for a read that ended unanswered.
