 ...
 * `spec/interaction-events.md:130-133` requires that no OpenLogo instruction and no handler block
 ...
 * (`spec/interaction-events.md:174-178`: `"stop"` is "a requested stop notification **before**
 ...
 *   whose clock offers no further yield** (`spec/interaction-events.md:220-226`), which #1039's
 ...
 * - **No `input` question is outstanding right now.** `spec/interaction-events.md:130-133` forbids
...
export interface RunController {
   ...
   * `spec/interaction-events.md:243-247` defines — `"left"`, `"space"`, `"a"`, and so on. Browser
   ...
   * **or** "an equivalent accessible action" (`spec/interaction-events.md:263-264`), which is why
 ...
 * registered" (`spec/interaction-events.md:142-144`), so the run's own trace stream is the record —
...
export function createRunController(
  ...
  // *within* it: `spec/interaction-events.md:130-133` blocks handlers only until the read finishes,
  ...
  // `spec/interaction-events.md:113-115` requires the most recent key/click state to be preserved.
  ...
  function presentPendingRead(read: PendingRead): void {
    ...
    read.host.present({ prompt: read.prompt }, (answer) => {
      ...
      if (answer === undefined) {
        ...
        // one other ending `spec/interaction-events.md:132-133` allows. Publish the cancellation
  ...
  function beginAttempt(
    ...
    executionHost.execute(request, (settlement) => {
      ...
      if (stopNotificationAttempt === attemptId) {
        ...
        // `spec/interaction-events.md:132-133` allows. Withdrawn **here**, keyed to this exact
   ...
   * `spec/interaction-events.md:220-226` recognises as holding the run open — so it is ended.
   *
   ...
   * `spec/interaction-events.md:220-226`'s "the run closes when the main line has finished", and a
  ...
  function programIsStillRunning(): boolean {
    if (attemptPending) {
      ...
      // where `spec/interaction-events.md:113-115` requires the most recent key/click state to be
   ...
   * it — no question may be outstanding (`spec/interaction-events.md:130-133` forbids running a
   ...
   * (`spec/interaction-events.md:174-178`) rather than input arriving at a live program, and the
  ...
  function stop(): void {
    ...
    // #952 — `spec/interaction-events.md:174-178` makes `"stop"` "a requested stop notification
