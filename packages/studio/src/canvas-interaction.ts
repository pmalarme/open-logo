 ...
 * `spec/interaction-events.md:287-288` says `on_click` runs "when the drawing surface is clicked
 ...
 * keyboard surface: `"enter"` and `"space"` are two of the key words `:240-244` names, so a learner
 ...
 * "does not standardize click coordinate reporters" (`:262-264`), so a click has no payload for a
