 ...
 * `spec/interaction-events.md:301-302` says `on_click` runs "when the drawing surface is clicked
 ...
 * keyboard surface: `"enter"` and `"space"` are two of the key words `:254-258` names, so a learner
 ...
 * "does not standardize click coordinate reporters" (`:276-278`), so a click has no payload for a
