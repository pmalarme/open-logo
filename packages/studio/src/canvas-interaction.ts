 ...
 * `spec/interaction-events.md:289-290` says `on_click` runs "when the drawing surface is clicked
 ...
 * keyboard surface: `"enter"` and `"space"` are two of the key words `:242-246` names, so a learner
 ...
 * "does not standardize click coordinate reporters" (`:264-266`), so a click has no payload for a
