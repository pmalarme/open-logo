...
/** A `when "stop"` program — the notification `spec/interaction-events.md:204-208` defines. */
...
test('#952: Stop delivers the when "stop" notification before termination', () => {
  ...
  assert.deepEqual(
    store.getState().output,
    ["bye"],
    'spec/interaction-events.md:204-208 — "stop" notifies the program before termination',
  );
...
test("#952: a delivery is refused while an input question is outstanding (spec/interaction-events.md:160-163)", () => {
...
test("#985: a program stays responsive for as many presses as the learner makes — the old bound was the counter's, not the program's", () => {
  ...
  //
  // `spec/interaction-events.md:433-436` names what stops delivery — "Cancellation stops future
  // handler delivery and sound scheduling" — and `:204-208` makes `"stop"` a *requested*
...
test("#952 (review finding 2): the recorded schedule does NOT depend on how fast the host settles", () => {
  ...
  // a schedule shaped by host timing, and a key dropped where spec/interaction-events.md:143-145
  ...
  assert.equal(
    ...
    "every press must reach the schedule — a press arriving while an execution is unsettled is " +
      "buffered, never dropped (spec/interaction-events.md:143-145 requires the most recent key " +
...
test("#976: a chain that has asked a question keeps accepting delivered input once the read finishes", () => {
  ...
  // than `spec/interaction-events.md:160-163`, which blocks handlers only "until the read finishes
  ...
  // `:160-163`. What must be asserted is therefore both halves: the key is delivered, AND the
...
test("#985: the boolean answers whether THIS press ran a handler — false for a key nothing names, and when the clock never ticks", () => {
  ...
  // tick 2") was pinning the synthetic counter's exhaustion artifact; `:433-436` names cancellation
...
test("#985: a handler that raises still reports true — the block-head marker precedes the failure", () => {
  ...
  // block-head marker `spec/interaction-events.md:154-155` mandates is monotonic on the error path.
  const store = OL.createStudioState({
...
test("#985 F4: a delivery replay resumes pacing at the tick already spent, not from zero", () => {
  ...
  // on every key press, at default speed, in precisely the `spec/interaction-events.md:168-170`
...
test("#985 F4: a long `wait` holds the run open while handlers drive the animation", () => {
  // `spec/interaction-events.md:168-170` — "This is what lets a program register its handlers and
  ...
  // Delivered WHILE the wait is outstanding — the property `:168-170` describes.
  assert.equal(
    controller.deliverKey("a"),
    true,
...
test('#976: Stop re-clamps its own notification — a `when "stop"` registered mid-program still fires', () => {
  ...
  // pre-termination notification `spec/interaction-events.md:204-208` requires is lost with no
...
test("#1039 AC1: a genuinely ENDED program refuses a key and a click — with no execution at all", () => {
  ...
  // (`spec/interaction-events.md:250-256` — "A handler does not extend the run's lifetime; that is
...
test("#1039 AC3: a `forever` that yields holds the run open the same way a long `wait` does", () => {
  // `spec/interaction-events.md:250-256` names `forever` and a long `wait` as *the* two ways a
 ...
 *   `"when"`, which is ungated by design (`spec/interaction-events.md:204-208`; pinned by
...
test("#1039: `wait 0` is a yield, so a program that ends on one is still not ended", () => {
  ...
  //
  // `rubber-duck` argued the other side: `:313-317` says `wait 0` yields "without adding a visible
  // delay" while `:165-170`/`:250-256` keep handlers alive with a *long* wait, so a trailing
...
test("#1039: the shapes this predicate deliberately still accepts", () => {
  ...
  // narrower than `spec/interaction-events.md:250-256`'s "the run closes once the main line has
  ...
  // 3. The main line has finished, which `:250-256` calls the run closing — but the clock yielded,
