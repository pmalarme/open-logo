...
/** A `when "stop"` program — the notification `spec/interaction-events.md:174-178` defines. */
...
test('#952: Stop delivers the when "stop" notification before termination', () => {
  ...
  assert.deepEqual(
    store.getState().output,
    ["bye"],
    'spec/interaction-events.md:174-178 — "stop" notifies the program before termination',
  );
...
test("#952: a delivery is refused while an input question is outstanding (spec/interaction-events.md:130-133)", () => {
...
test("#985: a program stays responsive for as many presses as the learner makes — the old bound was the counter's, not the program's", () => {
  ...
  //
  // `spec/interaction-events.md:403-406` names what stops delivery — "Cancellation stops future
...
test("#952 (review finding 2): the recorded schedule does NOT depend on how fast the host settles", () => {
  ...
  // a schedule shaped by host timing, and a key dropped where spec/interaction-events.md:113-115
  ...
  assert.equal(
    ...
    "every press must reach the schedule — a press arriving while an execution is unsettled is " +
      "buffered, never dropped (spec/interaction-events.md:113-115 requires the most recent key " +
...
test("#976: a chain that has asked a question keeps accepting delivered input once the read finishes", () => {
  ...
  // than `spec/interaction-events.md:130-133`, which blocks handlers only "until the read finishes
...
test("#985: a handler that raises still reports true — the block-head marker precedes the failure", () => {
  ...
  // block-head marker `spec/interaction-events.md:124-125` mandates is monotonic on the error path.
  const store = OL.createStudioState({
...
test("#985 F4: a delivery replay resumes pacing at the tick already spent, not from zero", () => {
  ...
  // on every key press, at default speed, in precisely the `spec/interaction-events.md:138-140`
...
test("#985 F4: a long `wait` holds the run open while handlers drive the animation", () => {
  // `spec/interaction-events.md:138-140` — "This is what lets a program register its handlers and
...
test('#976: Stop re-clamps its own notification — a `when "stop"` registered mid-program still fires', () => {
  ...
  // pre-termination notification `spec/interaction-events.md:174-178` requires is lost with no
...
test("#1039 AC1: a genuinely ENDED program refuses a key and a click — with no execution at all", () => {
  ...
  // (`spec/interaction-events.md:220-226` — "A handler does not extend the run's lifetime; that is
...
test("#1039 AC3: a `forever` that yields holds the run open the same way a long `wait` does", () => {
  // `spec/interaction-events.md:220-226` names `forever` and a long `wait` as *the* two ways a
 ...
 *   `"when"`, which is ungated by design (`spec/interaction-events.md:174-178`; pinned by
...
test("#1039: `wait 0` is a yield, so a program that ends on one is still not ended", () => {
  ...
  //
  // `rubber-duck` argued the other side: `:261-265` says `wait 0` yields "without adding a visible
  // delay" while `:113-118`/`:198-204` keep handlers alive with a *long* wait, so a trailing
...
test("#1039: the shapes this predicate deliberately still accepts", () => {
  ...
  // narrower than `spec/interaction-events.md:220-226`'s "the run closes once the main line has
