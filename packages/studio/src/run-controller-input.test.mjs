...
test("dismissing the question ends the read unanswered, cancelling the run (spec/interaction-events.md#trace-stream-integration)", () => {
  const store = OL.createStudioState({ source: ASK_NAME_SOURCE });
  const host = createTestPromptHost();
  const controller = OL.createRunController(store, { inputPrompt: host });
...
test("an answer that reads as a number is reported as one (spec/interaction-events.md#input-prompt-word), unchanged by the replay", () => {
  const store = OL.createStudioState({
