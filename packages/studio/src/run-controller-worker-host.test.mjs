...
test("the question is put to the learner over the picture the program has already drawn", () => {
  ...
  // draws the square and then asks. `spec/interaction-events.md#trace-stream-integration` permits exactly this
...
test("dismissing the question publishes the run's own cancellation, not an invented one", () => {
  ...
  // diagnostic the learner sees is the runtime's — `spec/interaction-events.md#trace-stream-integration`'s other
 ...
 * `spec/interaction-events.md#trace-stream-integration` ("until the read finishes").
 */
