 ...
 * `@openlogo/runtime`'s reader is **synchronous** — `spec/interaction-events.md#trace-stream-integration` requires
 ...
 * the run (`spec/interaction-events.md#trace-stream-integration`'s only other ending for a read).
 */
...
export interface InputPromptController extends InputPromptHost {
   ...
   * run (`spec/interaction-events.md#trace-stream-integration`). A no-op when nothing is outstanding.
   */
  cancel(): void;
}
