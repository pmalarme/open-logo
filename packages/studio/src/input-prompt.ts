 ...
 * `@openlogo/runtime`'s reader is **synchronous** — `spec/interaction-events.md:156-159` requires
 ...
 * the run (`spec/interaction-events.md:158-159`'s only other ending for a read).
 */
...
export interface InputPromptController extends InputPromptHost {
   ...
   * run (`spec/interaction-events.md:158-159`). A no-op when nothing is outstanding.
   */
  cancel(): void;
}
