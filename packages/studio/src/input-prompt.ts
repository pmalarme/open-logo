 ...
 * `@openlogo/runtime`'s reader is **synchronous** — `spec/interaction-events.md:160-163` requires
 ...
 * the run (`spec/interaction-events.md:162-163`'s only other ending for a read).
 */
...
export interface InputPromptController extends InputPromptHost {
   ...
   * run (`spec/interaction-events.md:162-163`). A no-op when nothing is outstanding.
   */
  cancel(): void;
}
