 ...
 * `@openlogo/runtime`'s reader is **synchronous** — `spec/interaction-events.md:154-157` requires
 ...
 * the run (`spec/interaction-events.md:156-157`'s only other ending for a read).
 */
...
export interface InputPromptController extends InputPromptHost {
   ...
   * run (`spec/interaction-events.md:156-157`). A no-op when nothing is outstanding.
   */
  cancel(): void;
}
