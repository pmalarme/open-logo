 ...
 * `@openlogo/runtime`'s reader is **synchronous** — `spec/interaction-events.md:130-133` requires
 ...
 * the run (`spec/interaction-events.md:132-133`'s only other ending for a read).
 */
...
export interface InputPromptController extends InputPromptHost {
   ...
   * run (`spec/interaction-events.md:132-133`). A no-op when nothing is outstanding.
   */
  cancel(): void;
}
