...
test("#748: ask with a turtle listed twice addresses it once — the block's command applies once, and the previous set is still restored", () => {
  ...
  // addresses :a ONCE (spec/turtles-and-sprites.md#addressing-model's "addressed set" + spec/turtles-and-sprites.md#per-turtle-state-and-turtle-commands's "once for each
  // addressed turtle"; turtle `==` is "Same turtle identity", spec/execution-model.md#equality-and-ordering). One move
