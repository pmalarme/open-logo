 ...
 *
 * `input` (issue #681) is the exact mirror image. `spec/interaction-events.md:168-171`: "`input` is
 ...
 * read (a `runWait`-style loop until an answer arrives). That is precisely what `:170-171` forbids,
 ...
 * (`spec/interaction-events.md:165-166`: "primitives without a more specific kind emit
 * `primitive`"). Emitted **after** the answer is in hand, matching the after-effect discipline the
 * whole stream follows (`:163-166`) and `wait`'s own "after the pause completes" rule. Like
 ...
 * (`spec/interaction-events.md:196-197`): "If the submitted text parses as an OpenLogo number
 ...
 * `spec/interaction-events.md:218-223` — "A `when` registration is **persistent**, exactly like
 ...
 * vendor-prefixed events `:215-216` permits.
 */
 ...
 * finished — is the spec's **fixed rate** rule (`spec/interaction-events.md:243-247`, maintainer
 ...
 *
 * The three flags implement the spec's queueing rule (`spec/interaction-events.md:249-256`): "If a
 ...
 * a `when` registration is persistent (`spec/interaction-events.md:218-223`).
 *
 ...
 * clock (`spec/interaction-events.md:243-247`). The arriving occurrence is put in the handler's
 * one-slot queue (`queued`); a second arrival while that slot is full **coalesces** into it, so the
 * queue never exceeds the spec's "at most one pending invocation" (`:249-256`). Tick `0` is never an
 ...
 * free**" (`spec/interaction-events.md:249-256`, maintainer ruling #984).
 *
 ...
 *
 * Neither loops until the queue is empty, and that is ruling 4 (`spec/interaction-events.md:258-264`):
 ...
 * "degrade[s] to running back to back" (`:253-255`) for as long as the program stays open, and
 ...
 * event carries only the primitive's *name* (`spec/interaction-events.md:180-182` — "Event
 ...
 * (`spec/interaction-events.md:218-223`), so an event delivered twice in one tick fires its handler
