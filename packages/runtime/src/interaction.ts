 ...
 *
 * `input` (issue #681) is the exact mirror image. `spec/interaction-events.md#trace-stream-integration`: "`input` is
 ...
 * read (a `runWait`-style loop until an answer arrives). That is precisely what `spec/interaction-events.md#trace-stream-integration` forbids,
 ...
 * `spec/interaction-events.md#time-ticks-and-handlers` makes the tick "an implementation-defined logical frame used by
 ...
 * whole stream follows (`spec/interaction-events.md#trace-stream-integration`) and `wait`'s own "after the pause completes" rule. Like
 ...
 * negative literal `-5` (`spec/grammar.md#lexical-form-and-encoding`: "A leading `-` directly before a numeral, when there
 ...
 * `spec/interaction-events.md#when-event-word-block` — "A `when` registration is **persistent**, exactly like
 ...
 * vendor-prefixed events `spec/interaction-events.md#when-event-word-block` permits.
 */
 ...
 * finished — is the spec's **fixed rate** rule (`spec/interaction-events.md#every-n-block`, maintainer
 ...
 *
 * The three flags implement the spec's queueing rule (`spec/interaction-events.md#every-n-block`): "If a
 ...
 * a `when` registration is persistent (`spec/interaction-events.md#when-event-word-block`).
 *
 ...
 * clock (`spec/interaction-events.md#every-n-block`). The arriving occurrence is put in the handler's
 * one-slot queue (`queued`); a second arrival while that slot is full **coalesces** into it, so the
 * queue never exceeds the spec's "at most one pending invocation" (`spec/interaction-events.md#every-n-block`). Tick `0` is never an
 ...
 * free**" (`spec/interaction-events.md#every-n-block`, maintainer ruling #984).
 *
 ...
 *
 * Neither loops until the queue is empty, and that is ruling 4 (`spec/interaction-events.md#every-n-block`):
 ...
 * "degrade[s] to running back to back" (`spec/interaction-events.md#every-n-block`) for as long as the program stays open, and
 ...
 * firing is a charged instruction (`spec/interaction-events.md#time-ticks-and-handlers`).
 */
 ...
 * event carries only the primitive's *name* (`spec/interaction-events.md#trace-stream-integration` — "Event
 ...
 * (`spec/interaction-events.md#when-event-word-block`), so an event delivered twice in one tick fires its handler
