 ...
 *
 * `input` (issue #681) is the exact mirror image. `spec/interaction-events.md:169-172`: "`input` is
 ...
 * read (a `runWait`-style loop until an answer arrives). That is precisely what `:171-172` forbids,
 ...
 * (`spec/interaction-events.md:166-167`: "primitives without a more specific kind emit
 * `primitive`"). Emitted **after** the answer is in hand, matching the after-effect discipline the
 * whole stream follows (`:164-167`) and `wait`'s own "after the pause completes" rule. Like
 ...
 * (`spec/interaction-events.md:197-198`): "If the submitted text parses as an OpenLogo number
 ...
 * `spec/interaction-events.md:219-224` — "A `when` registration is **persistent**, exactly like
 ...
 * vendor-prefixed events `:216-217` permits.
 */
 ...
 * finished — is the spec's **fixed rate** rule (`spec/interaction-events.md:244-248`, maintainer
 ...
 *
 * The three flags implement the spec's queueing rule (`spec/interaction-events.md:250-257`): "If a
 ...
 * a `when` registration is persistent (`spec/interaction-events.md:219-224`).
 *
 ...
 * clock (`spec/interaction-events.md:244-248`). The arriving occurrence is put in the handler's
 * one-slot queue (`queued`); a second arrival while that slot is full **coalesces** into it, so the
 * queue never exceeds the spec's "at most one pending invocation" (`:250-257`). Tick `0` is never an
 ...
 * free**" (`spec/interaction-events.md:250-257`, maintainer ruling #984).
 *
 ...
 *
 * Neither loops until the queue is empty, and that is ruling 4 (`spec/interaction-events.md:259-265`):
 ...
 * "degrade[s] to running back to back" (`:254-256`) for as long as the program stays open, and
 ...
 * event carries only the primitive's *name* (`spec/interaction-events.md:181-183` — "Event
 ...
 * (`spec/interaction-events.md:219-224`), so an event delivered twice in one tick fires its handler
