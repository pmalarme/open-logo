...
// code AND the same params (`spec/error-model.md:256-261`, "Localization boundary"). Heritage is
 ...
 * spec rule (`spec/execution-model.md:779-780` — "Used alone as a statement, its result is
    ...
    for (const callable of callables) {
      assert.equal(
        ...
        `alias ${alias}: params.callable must be the canonical "${canonical}", never the surface ` +
          `spelling (spec/error-model.md:256-261)`,
      );
    }
  }
});
      ...
      for (const [field, value] of Object.entries(actual.params)) {
        ...
        assert.deepEqual(
          ...
          `${twin.note}: diagnostic ${index} (${actual.code}) param "${field}" diverged with the ` +
            ...
            `params (spec/error-model.md:256-261); Heritage adds no new semantics ` +
