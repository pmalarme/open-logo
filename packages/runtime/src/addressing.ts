 ...
 * and restore.
 *
 * `spec/rendering.md#non-visual-state-descriptions` is normative — "Implementations with multiple turtles MUST identify the
 ...
 * restores (`spec/turtles-and-sprites.md#addressing-model`) the last such id is neither the active turtle nor the
 ...
 * closed (`spec/execution-model.md#trace-and-event-registry`: `kind` is "One registered event kind"), the spec is
 * maintainer-owned, and the only sanctioned unregistered kinds are vendor-namespaced extensions
 * (`vendor_name.event_name`, `spec/conformance.md#profile-dependency-dag, spec/conformance.md#extensions-and-feature-detection`) — which, being vendor-specific, must not
 * be recorded as portable behavior in a stack-neutral conformance fixture. `tell`, `ask`, and `each`
 * are C3 Sprites primitives (`spec/turtles-and-sprites.md#canonical-forms`) with no more specific event kind,
 * which is exactly what `primitive`, "the generic catch-all for a primitive without a more specific
 * event" (`spec/execution-model.md#trace-and-event-registry`), exists for — the same reading under which the Interaction
 * registration *forms* emit `primitive` (`spec/interaction-events.md#trace-stream-integration`). So the
 ...
 * `spec/execution-model.md#trace-and-event-registry`). `name` is the addressing primitive whose effect this is; the
 ...
 * (`spec/execution-model.md#trace-and-event-registry`): a later change of the addressed set must not retroactively
