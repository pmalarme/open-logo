 ...
 * `spec/execution-model.md:1001-1012` calls for: a mutable map (like `instructionCount`/`addressing`,
...
export interface Environment {
   ...
   * `spec/interaction-events.md:246-261`). While set, {@link executeStatements} runs it before each
   ...
   * take and the read ends the only other way `spec/interaction-events.md:167-168` allows — as a
   ...
   * `spec/interaction-events.md:165-168`'s "MUST NOT run new OpenLogo instructions or event handler
...
// --- Loop/comprehension binder helpers (spec/execution-model.md:795-799) --------------------
//
 ...
 * a `for`/comprehension binder's own name(s) — `spec/execution-model.md:795-797` ("body-local
 ...
 * destructuring pattern destructures it positionally (`spec/execution-model.md:795-799`). A list
 ...
 * every alias (`spec/data-structures.md:47`, `spec/execution-model.md:831-841`). A target that
...
/**
 * Execute `add value to target` (`spec/data-structures.md:79`, `spec/execution-model.md:831-841`):
...
//
// spec/execution-model.md:843-870. `==`/`!=` compare any two values to a boolean and never
// raise; ordering is defined only for two numbers or two words and raises `ol-type` otherwise.

/**
 * The canonical printed form of a number (`spec/execution-model.md:19,858-860`): whole values
...
function primitivePrintedForm(value: OLValue): string | undefined {
  ...
  if (value instanceof OLTurtle) {
    ...
    // (`spec/turtles-and-sprites.md:13`, `spec/execution-model.md:900`): a turtle is an opaque
 ...
 * original (`spec/execution-model.md:900`). So a turtle is copied by keeping the same reference,
...
/**
 * Normative `==` for OpenLogo's value types (`spec/execution-model.md:843-870` matrix): numeric
...
function equalRec(a: OLValue, b: OLValue, inProgress: EqualityMemo): boolean {
  ...
  if (a instanceof OLTurtle) {
    // Turtles compare by identity, never by state (`spec/execution-model.md:900`): a turtle equals
 ...
 * (`spec/execution-model.md:862-866`). `inProgress` holds the reference pairs currently on the
...
/**
 * Structural dict equality (issue #322, `spec/execution-model.md:854`): same key set and pairwise
 ...
 * (`spec/execution-model.md:869`). `Array.from` iterates by code point (not UTF-16 code unit),
 ...
 * (`spec/execution-model.md:868-870`). When the left operand is itself non-orderable
...
// #234; spec/commands.md "Words and lists", spec/execution-model.md:807-842) ---------------------
//
...
// are shared, only the outer array is copied (`spec/execution-model.md:807-842`'s
 ...
 * (`spec/commands.md` "fput"/"lput"; `spec/execution-model.md:807-842` — never mutates `list`).
 ...
 * is positive — matching `spec/execution-model.md:898` and verified against the spec's own worked
 ...
 * `spec/interaction-events.md:183-194`): a Kind-R reporter taking one prompt that displays the
 * prompt, waits for the learner to enter one value, and reports it as a word or a number. It is
 * "the only blocking read in OpenLogo v0.1 and belongs to this profile, not Core" (`:191-192`,
 ...
 *      can never finish, so it takes the only other ending `:167-168` allows and the program is
 ...
 *      `spec/interaction-events.md:193-194` ({@link interpretSubmittedText}).
 *
 * The **blocking** property (`:165-168` — while the read waits, no new OpenLogo instruction and no
 ...
 * enters (`spec/interaction-events.md:191`). Reports the submitted text, or `undefined` when the
 ...
 * script.
 *
 * Either way the read is **synchronous**, which is how `spec/interaction-events.md:165-168`'s "MUST
...
// --- Comprehensions: map / filter / reduce (spec/execution-model.md:727-839, issue #105) ------
//
// Comprehensions are value-producing *expressions* usable anywhere an expression is
// (`spec/execution-model.md:727-731`), so — unlike a procedure body, which can contain arbitrary
...
/**
 * Evaluate a `map`/`filter`/`reduce` comprehension (`spec/execution-model.md:727-839`, worked
 * examples `:1055-1101`): binder-duplicate check first ({@link comprehensionDuplicateBinder}), then
 ...
 * empty, `spec/execution-model.md:762`).
 */
/**
 ...
 * `spec/interaction-events.md:246-261`).
 *
