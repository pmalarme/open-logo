 ...
 * `spec/execution-model.md:982-993` calls for: a mutable map (like `instructionCount`/`addressing`,
...
export interface Environment {
   ...
   * `spec/interaction-events.md:241-256`). While set, {@link executeStatements} runs it before each
   ...
   * take and the read ends the only other way `spec/interaction-events.md:162-163` allows — as a
   ...
   * `spec/interaction-events.md:160-163`'s "MUST NOT run new OpenLogo instructions or event handler
...
// --- Loop/comprehension binder helpers (spec/execution-model.md:776-780) --------------------
//
 ...
 * a `for`/comprehension binder's own name(s) — `spec/execution-model.md:776-778` ("body-local
 ...
 * destructuring pattern destructures it positionally (`spec/execution-model.md:776-780`). A list
 ...
 * every alias (`spec/data-structures.md:47`, `spec/execution-model.md:812-822`). A target that
...
/**
 * Execute `add value to target` (`spec/data-structures.md:79`, `spec/execution-model.md:812-822`):
...
//
// spec/execution-model.md:824-851. `==`/`!=` compare any two values to a boolean and never
// raise; ordering is defined only for two numbers or two words and raises `ol-type` otherwise.

/**
 * The canonical printed form of a number (`spec/execution-model.md:19,839-841`): whole values
...
function primitivePrintedForm(value: OLValue): string | undefined {
  ...
  if (value instanceof OLTurtle) {
    ...
    // (`spec/turtles-and-sprites.md:13`, `spec/execution-model.md:881`): a turtle is an opaque
 ...
 * original (`spec/execution-model.md:881`). So a turtle is copied by keeping the same reference,
...
/**
 * Normative `==` for OpenLogo's value types (`spec/execution-model.md:824-851` matrix): numeric
...
function equalRec(a: OLValue, b: OLValue, inProgress: EqualityMemo): boolean {
  ...
  if (a instanceof OLTurtle) {
    // Turtles compare by identity, never by state (`spec/execution-model.md:881`): a turtle equals
 ...
 * (`spec/execution-model.md:843-847`). `inProgress` holds the reference pairs currently on the
...
/**
 * Structural dict equality (issue #322, `spec/execution-model.md:835`): same key set and pairwise
 ...
 * (`spec/execution-model.md:850`). `Array.from` iterates by code point (not UTF-16 code unit),
 ...
 * (`spec/execution-model.md:849-851`). When the left operand is itself non-orderable
...
// #234; spec/commands.md "Words and lists", spec/execution-model.md:788-823) ---------------------
//
...
// are shared, only the outer array is copied (`spec/execution-model.md:788-823`'s
 ...
 * (`spec/commands.md` "fput"/"lput"; `spec/execution-model.md:788-823` — never mutates `list`).
 ...
 * is positive — matching `spec/execution-model.md:879` and verified against the spec's own worked
 ...
 * `spec/interaction-events.md:178-189`): a Kind-R reporter taking one prompt that displays the
 * prompt, waits for the learner to enter one value, and reports it as a word or a number. It is
 * "the only blocking read in OpenLogo v0.1 and belongs to this profile, not Core" (`:186-187`,
 ...
 *      can never finish, so it takes the only other ending `:162-163` allows and the program is
 ...
 *      `spec/interaction-events.md:188-189` ({@link interpretSubmittedText}).
 *
 * The **blocking** property (`:160-163` — while the read waits, no new OpenLogo instruction and no
 ...
 * enters (`spec/interaction-events.md:186`). Reports the submitted text, or `undefined` when the
 ...
 * script.
 *
 * Either way the read is **synchronous**, which is how `spec/interaction-events.md:160-163`'s "MUST
...
// --- Comprehensions: map / filter / reduce (spec/execution-model.md:708-820, issue #105) ------
//
// Comprehensions are value-producing *expressions* usable anywhere an expression is
// (`spec/execution-model.md:708-712`), so — unlike a procedure body, which can contain arbitrary
...
/**
 * Evaluate a `map`/`filter`/`reduce` comprehension (`spec/execution-model.md:708-820`, worked
 * examples `:1036-1082`): binder-duplicate check first ({@link comprehensionDuplicateBinder}), then
 ...
 * empty, `spec/execution-model.md:743`).
 */
/**
 ...
 * `spec/interaction-events.md:241-256`).
 *
