...
// catch-all for a primitive without a more specific kind, spec/execution-model.md:802 — no new event
...
test("an addressing event is never stamped with an envelope turtle_id (it describes a set)", () => {
  // spec/execution-model.md:737 — `turtle-id` is "present only when the event is turtle-specific".
...
test("each addressing event reports its own set: a later tell does not rewrite an earlier event", () => {
  // spec/execution-model.md:751-760 — an effect payload is a point-in-time snapshot, not a live
