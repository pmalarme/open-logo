 ...
 * only stopping its own loop (`spec/execution-model.md:706-721`). {@link runProcedure} is the
 ...
 * `spec/execution-model.md:952-953` requires, reporting the position change and heading. A
 ...
 *
 * Movement math is `spec/execution-model.md:905-906`'s `(x + d·sin h, y + d·cos h)`: heading `0`
...
function executeTurtleMoveCall(
  ...
  if (!Number.isFinite(distance.value)) {
    ...
    // diagnostic (`spec/execution-model.md:877` — "OpenLogo never exposes NaN or Infinity as
 ...
 * counter-clockwise, i.e. `left` — `spec/execution-model.md:897`) and emit the `turn` effect-event
 * `spec/execution-model.md:954` requires (`{from, to}`, both headings in degrees). The new heading
 * is normalized to `[0,360)` (`spec/execution-model.md:898`) — never left negative or `>= 360`.
 *
 ...
 * delta, since `right`/clockwise is positive — `spec/execution-model.md:897`), then delegated to
...
function homeTurtleForClearScreen(
  ...
  // point-in-time snapshot at the moment of emission (`spec/execution-model.md:1012`) and each
 ...
 * `spec/execution-model.md:877`). Returns an {@link ExecSignal} to halt on, or `undefined` for
 * {@link executeStatements} to `continue` on success (including the "left un-evaluated" case for
 * an unsupported argument expression, mirroring `forward`/`back`'s handling).
 *
 * Deliberately a separate, non-inlined function — same stack-frame-size rationale documented on
...
function executeTurtleHeadingCall(
  ...
  if (!Number.isFinite(angle.value)) {
    ...
    // turtle's heading instead of raising a diagnostic (`spec/execution-model.md:877`).
    return halt(
...
/**
 * Is `statement` a call to `set_tempo` (issue #689; `spec/interaction-events.md:343-356`). Same
 ...
 * numeric argument (`ol-not-enough-inputs`/`ol-too-many-inputs`/`ol-type` otherwise, via
 * {@link requireNumber}), which must additionally be positive and finite
 * (`spec/interaction-events.md:346` — "one positive number") or `runtimeDiag.nonPositiveTempo`
...
/**
 * Is `statement` a call to `beep` (issue #689; `spec/interaction-events.md:393-408`). Same
 ...
 * `beep` schedules "one short implementation-defined alert sound" (`spec/interaction-events.md:401`)
...
/**
 * Is `statement` a call to `note` (issue #690; `spec/interaction-events.md:358-375`). Same
...
/**
 * Is `statement` a call to `rest` (issue #690; `spec/interaction-events.md:410-425`). Same
...
/**
 * Is `statement` a call to `play` (issue #691; `spec/interaction-events.md:377-391`). Same
 * shape/convention as {@link isSoundSetTempoCall} — an ordinary Sound-profile primitive-name match
 ...
 * MUST be even" (`spec/interaction-events.md:385-387`) — an odd length raises `ol-range`
 ...
 * `spec/interaction-events.md:351-352`) — and emits exactly one
...
function executeWaitCall(
  ...
  // order (`when` → `on_key` → `on_click` → due `every`, `spec/interaction-events.md:141-146`) —
  ...
  // top-level instructions after the `wait` being deferred (`spec/interaction-events.md:170-175`).
  //
 ...
 * named event occurs, once per occurrence" (`spec/interaction-events.md:215-220`, maintainer ruling
 ...
 * earlier already caught it. That is what keeps persistence (`spec/interaction-events.md:215-220`,
 ...
 * `spec/interaction-events.md:246-261`), returning a halting {@link ExecSignal} or `undefined`.
 *
 ...
 * set**"), and turtle `==` is "Same turtle identity" (`spec/execution-model.md:900`), so a turtle
 ...
 * invocation
 * (`spec/interaction-events.md:246-253`). Returns the body's
...
function dispatchDueHandlers(
  ...
  // (`spec/interaction-events.md:246-253`) — NOT at whatever later checkpoint the program happens to
  ...
  // extend it" (`spec/interaction-events.md:255-261`). A drained invocation whose own body outruns
 ...
 * and `spec/execution-model.md:956-957`: "`randomize` with no input uses an implementation seed").
...
function executeRandomizeCall(
  ...
  if (statement.args.length === 0) {
    ...
    // rather than by reading the wall clock. `spec/execution-model.md:956-957` ("`randomize` with
 ...
 * progression (`spec/execution-model.md:1001-1012`, "for the SAME target") could never observe two
 ...
 * `spec/execution-model.md:1001-1012`'s "observable ordering ... for a given target-source-span
 * value" requirement.
 */
 ...
 * (`spec/execution-model.md:749`, `spec/error-model.md:123`). `operation` names the leading
 ...
 * procedure exit the whole procedure, not just that loop (`spec/execution-model.md:706-721`).
 ...
 * (`spec/execution-model.md:1135-1173`).
 *
 ...
 * (`spec/execution-model.md:706-721`). Wired onto every execution `Environment`'s
 ...
 * otherwise (`spec/execution-model.md:749`) — and runs exactly one branch: `thenBody` when
 ...
 * exact order (`spec/execution-model.md:749-751`): a non-whole-number count raises `ol-type`
 ...
 * (`spec/execution-model.md:757-758`; Core `for ... in` is list-only, dict iteration is a later
 ...
 * `spec/execution-model.md:820-821`). A duplicate name within one destructuring pattern
 ...
 * negative step while it is at least `to` (`spec/execution-model.md:752-756`). A step pointing
 ...
 *
 * Both loops' binders are fresh **body-local** bindings (`spec/execution-model.md:340,1230`): each
 ...
 * not just that loop (`spec/execution-model.md:706-721`).
 *
...
function executeStatements(
  ...
  for (const rawStatement of statements) {
    // The main line's statement boundary (ruling #984, `spec/interaction-events.md:246-261`). Runs
 ...
 * profile's `hint` progression (`spec/execution-model.md:1001-1012`) starts over — every target
...
function createExecutionEnvironment(
  ...
  return {
    ...
    // `hostInput.read` can read what has already been emitted — `spec/interaction-events.md:165-167`
 ...
 * top-level statements** (maintainer ruling #984, `spec/interaction-events.md:246-261`).
 *
