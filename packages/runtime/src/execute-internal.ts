 ...
 * only stopping its own loop (`spec/execution-model.md:374-380`). {@link runProcedure} is the
 ...
 * `spec/execution-model.md:609-610` requires, reporting the position change and heading. A
 ...
 *
 * Movement math is `spec/execution-model.md:562-563`'s `(x + d·sin h, y + d·cos h)`: heading `0`
...
function executeTurtleMoveCall(
  ...
  if (!Number.isFinite(distance.value)) {
    ...
    // diagnostic (`spec/execution-model.md:534` — "OpenLogo never exposes NaN or Infinity as
 ...
 * counter-clockwise, i.e. `left` — `spec/execution-model.md:758`) and emit the `turn` effect-event
 * `spec/execution-model.md:852` requires (`{from, to}`, both headings in degrees). The new heading
 * is normalized to `[0,360)` (`spec/execution-model.md:759`) — never left negative or `>= 360`.
 *
 ...
 * delta, since `right`/clockwise is positive — `spec/execution-model.md:554`), then delegated to
...
function homeTurtleForClearScreen(
  ...
  // point-in-time snapshot at the moment of emission (`spec/execution-model.md:669`) and each
 ...
 * `spec/execution-model.md:534`). Returns an {@link ExecSignal} to halt on, or `undefined` for
 * {@link executeStatements} to `continue` on success (including the "left un-evaluated" case for
 * an unsupported argument expression, mirroring `forward`/`back`'s handling).
 *
 * Deliberately a separate, non-inlined function — same stack-frame-size rationale documented on
...
function executeTurtleHeadingCall(
  ...
  if (!Number.isFinite(angle.value)) {
    ...
    // turtle's heading instead of raising a diagnostic (`spec/execution-model.md:534`).
    return halt(
 ...
 * set**"), and turtle `==` is "Same turtle identity" (`spec/execution-model.md:692`), so a turtle
 ...
 * and `spec/execution-model.md:613-614`: "`randomize` with no input uses an implementation seed").
...
function executeRandomizeCall(
  ...
  if (statement.args.length === 0) {
    ...
    // rather than by reading the wall clock. `spec/execution-model.md:613-614` ("`randomize` with
 ...
 * progression (`spec/execution-model.md:658-669`, "for the SAME target") could never observe two
 ...
 * `spec/execution-model.md:658-669`'s "observable ordering ... for a given target-source-span
 * value" requirement.
 */
 ...
 * (`spec/execution-model.md:395`, `spec/error-model.md:123`). `operation` names the leading
 ...
 * procedure exit the whole procedure, not just that loop (`spec/execution-model.md:374-380`).
 ...
 * (`spec/execution-model.md:792-830`).
 *
 ...
 * (`spec/execution-model.md:374-380`). Wired onto every execution `Environment`'s
 ...
 * otherwise (`spec/execution-model.md:395`) — and runs exactly one branch: `thenBody` when
 ...
 * exact order (`spec/execution-model.md:395-397`): a non-whole-number count raises `ol-type`
 ...
 * (`spec/execution-model.md:403-404`; Core `for ... in` is list-only, dict iteration is a later
 ...
 * `spec/execution-model.md:477-478`). A duplicate name within one destructuring pattern
 ...
 * negative step while it is at least `to` (`spec/execution-model.md:398-402`). A step pointing
 ...
 *
 * Both loops' binders are fresh **body-local** bindings (`spec/execution-model.md:340,376`): each
 ...
 * not just that loop (`spec/execution-model.md:374-380`).
 *
 ...
 * profile's `hint` progression (`spec/execution-model.md:658-669`) starts over — every target
