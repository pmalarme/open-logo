 ...
 * only stopping its own loop (`spec/execution-model.md:618-624`). {@link runProcedure} is the
 ...
 * `spec/execution-model.md:855-856` requires, reporting the position change and heading. A
 ...
 *
 * Movement math is `spec/execution-model.md:808-809`'s `(x + d·sin h, y + d·cos h)`: heading `0`
...
function executeTurtleMoveCall(
  ...
  if (!Number.isFinite(distance.value)) {
    ...
    // diagnostic (`spec/execution-model.md:780` — "OpenLogo never exposes NaN or Infinity as
 ...
 * counter-clockwise, i.e. `left` — `spec/execution-model.md:800`) and emit the `turn` effect-event
 * `spec/execution-model.md:857` requires (`{from, to}`, both headings in degrees). The new heading
 * is normalized to `[0,360)` (`spec/execution-model.md:801`) — never left negative or `>= 360`.
 *
 ...
 * delta, since `right`/clockwise is positive — `spec/execution-model.md:800`), then delegated to
...
function homeTurtleForClearScreen(
  ...
  // point-in-time snapshot at the moment of emission (`spec/execution-model.md:915`) and each
 ...
 * `spec/execution-model.md:780`). Returns an {@link ExecSignal} to halt on, or `undefined` for
 * {@link executeStatements} to `continue` on success (including the "left un-evaluated" case for
 * an unsupported argument expression, mirroring `forward`/`back`'s handling).
 *
 * Deliberately a separate, non-inlined function — same stack-frame-size rationale documented on
...
function executeTurtleHeadingCall(
  ...
  if (!Number.isFinite(angle.value)) {
    ...
    // turtle's heading instead of raising a diagnostic (`spec/execution-model.md:780`).
    return halt(
...
/**
 * Is `statement` a call to `set_tempo` (issue #689; `spec/interaction-events.md:332-345`). Same
 ...
 * numeric argument (`ol-not-enough-inputs`/`ol-too-many-inputs`/`ol-type` otherwise, via
 * {@link requireNumber}), which must additionally be positive and finite
 * (`spec/interaction-events.md:335` — "one positive number") or `runtimeDiag.nonPositiveTempo`
...
/**
 * Is `statement` a call to `beep` (issue #689; `spec/interaction-events.md:382-397`). Same
 ...
 * `beep` schedules "one short implementation-defined alert sound" (`spec/interaction-events.md:390`)
...
/**
 * Is `statement` a call to `note` (issue #690; `spec/interaction-events.md:347-364`). Same
...
/**
 * Is `statement` a call to `rest` (issue #690; `spec/interaction-events.md:399-414`). Same
...
/**
 * Is `statement` a call to `play` (issue #691; `spec/interaction-events.md:366-380`). Same
 * shape/convention as {@link isSoundSetTempoCall} — an ordinary Sound-profile primitive-name match
 ...
 * MUST be even" (`spec/interaction-events.md:374-376`) — an odd length raises `ol-range`
 ...
 * `spec/interaction-events.md:340-341`) — and emits exactly one
...
function executeWaitCall(
  ...
  // order (`when` → `on_key` → `on_click` → due `every`, `spec/interaction-events.md:130-135`) —
  ...
  // top-level instructions after the `wait` being deferred (`spec/interaction-events.md:159-164`).
  //
 ...
 * named event occurs, once per occurrence" (`spec/interaction-events.md:204-209`, maintainer ruling
 ...
 * earlier already caught it. That is what keeps persistence (`spec/interaction-events.md:204-209`,
 ...
 * `spec/interaction-events.md:235-250`), returning a halting {@link ExecSignal} or `undefined`.
 *
 ...
 * set**"), and turtle `==` is "Same turtle identity" (`spec/execution-model.md:803`), so a turtle
 ...
 * invocation
 * (`spec/interaction-events.md:235-242`). Returns the body's
...
function dispatchDueHandlers(
  ...
  // (`spec/interaction-events.md:235-242`) — NOT at whatever later checkpoint the program happens to
  ...
  // extend it" (`spec/interaction-events.md:244-250`). A drained invocation whose own body outruns
 ...
 * and `spec/execution-model.md:859-860`: "`randomize` with no input uses an implementation seed").
...
function executeRandomizeCall(
  ...
  if (statement.args.length === 0) {
    ...
    // rather than by reading the wall clock. `spec/execution-model.md:859-860` ("`randomize` with
 ...
 * progression (`spec/execution-model.md:904-915`, "for the SAME target") could never observe two
 ...
 * `spec/execution-model.md:904-915`'s "observable ordering ... for a given target-source-span
 * value" requirement.
 */
 ...
 * (`spec/execution-model.md:652`, `spec/error-model.md:123`). `operation` names the leading
 ...
 * procedure exit the whole procedure, not just that loop (`spec/execution-model.md:618-624`).
 ...
 * (`spec/execution-model.md:1038-1076`).
 *
 ...
 * (`spec/execution-model.md:618-624`). Wired onto every execution `Environment`'s
 ...
 * otherwise (`spec/execution-model.md:652`) — and runs exactly one branch: `thenBody` when
 ...
 * exact order (`spec/execution-model.md:652-654`): a non-whole-number count raises `ol-type`
 ...
 * (`spec/execution-model.md:660-661`; Core `for ... in` is list-only, dict iteration is a later
 ...
 * `spec/execution-model.md:723-724`). A duplicate name within one destructuring pattern
 ...
 * negative step while it is at least `to` (`spec/execution-model.md:655-659`). A step pointing
 ...
 *
 * Both loops' binders are fresh **body-local** bindings (`spec/execution-model.md:340,1133`): each
 ...
 * not just that loop (`spec/execution-model.md:618-624`).
 *
...
function executeStatements(
  ...
  for (const rawStatement of statements) {
    // The main line's statement boundary (ruling #984, `spec/interaction-events.md:235-250`). Runs
 ...
 * profile's `hint` progression (`spec/execution-model.md:904-915`) starts over — every target
...
function createExecutionEnvironment(
  ...
  return {
    ...
    // `hostInput.read` can read what has already been emitted — `spec/interaction-events.md:154-156`
 ...
 * top-level statements** (maintainer ruling #984, `spec/interaction-events.md:235-250`).
 *
