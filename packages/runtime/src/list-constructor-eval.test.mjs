...
// code, same params, same span, and no list reaches `print`. These tests assert every field
// `spec/error-model.md#diagnostic-shape` requires **except `message`**, which that section
// defines as localizable prose generated from the code and params — so pinning its English would
// pin a translation, not a behaviour.
//
// Second, evaluation stops **at** the failing argument. That a runtime error stops the program
// is normative — `spec/commands.md#throw` says a thrown error "stops the program like any other
// runtime error", and these diagnostics carry `stage: "runtime"`. What the spec does **not** fix
// anywhere is the evaluation order of an ordinary argument list: the only left-to-right rules it
// states are for postfix chains, `and`/`or`, and `reduce`. So the two order tests below
...
// Asserts every field `spec/error-model.md#diagnostic-shape` requires except the localizable
// `message`: code, params and span identify the diagnostic, and `stage`/`severity` are checked
// so a reconstructed diagnostic that changed either would not pass for a propagated one.
