...
// cannot use it at all: a reporter would be evaluated, so the argument has to be a bare variable
// read, and the winning diagnostic's own `params` are the only evidence. What all three observe
// is confined to the failing construct: none places an instruction after the failing one, so
// whole-program termination is not measured here either.
//
