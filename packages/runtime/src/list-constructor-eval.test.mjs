...
// keys on. Note the scope: these tests observe continuation *within* the failing construct,
// which is exactly what that sentence governs. Whole-program termination is a stronger claim
// (`spec/commands.md#throw` states it) and is not measured here — neither order test places an
// instruction after the failing one.
//
...
// non-blankness, never for its text, since the spec makes it localizable prose. Trimmed rather
// than raw length, so a whitespace-only message fails too; both that and an emptied message are
// load-bearing, each defeating the weaker check it replaced.
