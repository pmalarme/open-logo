...
// rest: an implementation that evaluates every argument and *then* reports the earliest failure
// still never continues normally at the offending construct, yet the test below rejects it. So
// these tests deliberately pin current runtime behaviour, not a contract. They are asserted
