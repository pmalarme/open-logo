...
// non-emptiness only, never by its text. That section calls it "Localizable learner-facing prose
...
// unkilled.
//
// Second, evaluation stops **at** the failing argument. Two citations carry different halves of
// that, and neither is redundant. `spec/error-model.md#severity` supplies the generality: of
// `severity: error` it says "execution cannot continue normally at the offending construct", and
// these diagnostics carry `severity: "error"` and `stage: "runtime"` — both asserted below, so
// the tests establish the precondition the rule keys on. But "at the offending construct"
// underdetermines whole-program termination, which is what the two order tests actually observe;
// `spec/commands.md#throw` supplies that strength, describing a thrown error as one that "stops
// the program like any other runtime error".
//
...
// changed either would not pass for a propagated one; and `message` is checked for presence and
// non-emptiness, never for its text, since the spec makes it localizable prose. Non-emptiness is
// the strongest translation-invariant check available — every translation of a real message is
// non-empty — and it is load-bearing: emptying the field defeats a presence-only assertion.
