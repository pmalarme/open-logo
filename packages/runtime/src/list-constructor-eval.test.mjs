...
// unkilled. That nothing is printed rests on `spec/error-model.md#severity`, which says of
// `severity: error` that "execution cannot continue normally at the offending construct".
//
// Second, evaluation stops at the failing argument and the arguments after it never run.
// **Neither half is a language requirement**, and the header says so because the tests cannot.
// The spec fixes the evaluation order of an ordinary argument list nowhere — the only
// left-to-right rules it states are for postfix chains, `and`/`or`, and `reduce` — and the
// `#severity` sentence above does not supply the rest: an implementation that evaluates every
// argument and *then* reports the earliest failure still never continues normally at the
// offending construct, yet test 4 rejects it. So both order tests deliberately pin current
// runtime behaviour, not a contract. They are asserted observably, with a reporter that prints,
// because an argument like `reverse 5` fails without leaving a trace — it can show which
// diagnostic wins, but never whether the other argument ran. What they observe is confined to
// the failing construct: neither places an instruction after the failing one, so whole-program
// termination is not measured here.
//
...
test("(list …) builds the list when every argument evaluates cleanly", () => {
  ...
  // refused every argument. `noisy` prints, then the built list prints. The two elements differ
  // so the assertion pins their order too — `[2 1]`, not a constructor that prepends.
