...
// deliberately pin current runtime behaviour, not a contract. Two of the three use a reporter
// that prints, because an argument like `reverse 5` fails without leaving a trace — it can show
// which diagnostic wins, but never whether the other argument ran. The earlier-failure-wins test
// uses bare variable reads instead, because a print trace would show which arguments *ran*, not
// which failure was *returned*; the winning diagnostic's `params` are what carry that. What all
// three observe is confined to the failing construct: none places an instruction after the
...
test("(list …) reports the earlier failure when two arguments both fail", () => {
  ...
  // past the first failure and returned the last one would pass them all. Bare variable reads
  // rather than `noisy` here, because a print trace would show which arguments ran, not which
  // failure was returned — `alpha` in the params is what makes earlier-wins observable.
