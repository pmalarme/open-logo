...
test("a persistent `when` handler fires once per occurrence when its event is pending twice", () => {
  // `when` is PERSISTENT (maintainer ruling #984, `spec/interaction-events.md:206-211`): its block
...
test("a persistent `when` handler fires again when its event occurs on a later tick", () => {
  ...
  // handler on BOTH ticks. A vendor-prefixed word (`spec/interaction-events.md:203-204`) is used
