...
// `spec/interaction-events.md#trace-stream-integration`):
//
...
//      (`spec/interaction-events.md#time-ticks-and-handlers`): a later `wait` still delivers what the reads passed
//      over.
...
test("while a read is OUTSTANDING, no further instruction and no handler block has run", () => {
  // The strongest form of `spec/interaction-events.md#trace-stream-integration`, and the one the scripted-answer
...
test("the reader receives the prompt word verbatim — this is how a host shows it", () => {
  // `spec/interaction-events.md#input-prompt-word`: "`input` displays the prompt and waits for the learner to
  // enter one value." The runtime's half of "displays" is handing the host exactly the text a
  // learner sees. Since the #768 ruling the prompt is always a `word` (`spec/interaction-events.md#input-prompt-word`), so that text IS the
...
test("a reader that declines to answer cancels the run — the read's other ending, on demand", () => {
  // `spec/interaction-events.md#trace-stream-integration` again: "until the read finishes or the program is cancelled". A live host that
...
test("a reader's answer is classified by the same number-vs-word rule as a scripted one", () => {
  // One meaning, two hosts: `spec/interaction-events.md#input-prompt-word` applies to whatever text the
...
test("no handler block of ANY kind runs across a read — when, on_key, and on_click alike", () => {
  // `spec/interaction-events.md#time-ticks-and-handlers` lists four handler kinds; the MUST names "event handler
...
test("reads leave outstanding input OUTSTANDING — a later wait still delivers what they declined to", () => {
  ...
  // key" from "the read quietly consumed and discarded it". `spec/interaction-events.md#time-ticks-and-handlers`
...
test("the next instruction's events all follow the read's primitive event", () => {
  // The other clause of `spec/interaction-events.md#trace-stream-integration` — "MUST NOT run new OpenLogo instructions" — as the event stream
...
test("a cancelled run stops at the read — the spec's other ending — and runs nothing after it", () => {
  // `spec/interaction-events.md#trace-stream-integration` allows exactly two ways out of a blocking read: "until the read finishes or the
