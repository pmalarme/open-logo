 ...
 * - `hostInput` (issue #686, slice I7 — `spec/interaction-events.md:152-154`) — a {@link HostInput}
 ...
 *   same-tick order (`when` → `on_key` → `on_click` → due `every`, `spec/interaction-events.md:145-150`)
 *   — from a pure headless `execute()` call with no real input device. The spec (`:152-154`) requires
 ...
 *   show the learner a question over a blank canvas. `spec/interaction-events.md:169-171` explicitly
 ...
 *   (`spec/interaction-events.md:181-183`), never its key word, so answering it previously meant
...
export interface HostInput {
   ...
   * `spec/interaction-events.md:197-198` (text that parses as an OpenLogo number literal reports a
   ...
   * other ending `spec/interaction-events.md:171-172` allows and the program is cancelled with
   ...
   * text (classified by `spec/interaction-events.md:197-198` exactly as a scripted answer is);
   * returning `undefined` means the host cannot or will not answer, which ends the read the only
   * other way `:171-172` allows — the program is cancelled.
   *
   * This is how a real host both **displays the prompt** (`:195`) and holds the read open: the
   * reader IS the outstanding read, so a caller can observe, from inside it, that no further
   * OpenLogo instruction and no event handler block has run — the normative MUST at `:169-172`,
 ...
 * learner (`spec/interaction-events.md:190`: the prompt MUST be a `word`) — and reports
 * the text the learner submitted, or `undefined` to leave the read unanswered and cancel the run.
 * Synchronous by design: `spec/interaction-events.md:169-172` requires that no OpenLogo instruction
