 ...
 * per statement (`spec/execution-model.md:863-904` — the `instruction` event is the unit of
 ...
 * (with an optional `else`) and `while` their runtime meaning (`spec/execution-model.md:656-660`):
 ...
 * validates its count TYPE then RANGE, in that order (`spec/execution-model.md:656-660`) —
 ...
 * - `hostInput` (issue #686, slice I7 — `spec/interaction-events.md:139-141`) — a {@link HostInput}
 ...
 *   same-tick order (`when` → `on_key` → `on_click` → due `every`, `spec/interaction-events.md:132-137`)
 *   — from a pure headless `execute()` call with no real input device. The spec (`:139-141`) requires
 ...
 *   show the learner a question over a blank canvas. `spec/interaction-events.md:156-158` explicitly
 ...
 *   (`spec/interaction-events.md:168-170`), never its key word, so answering it previously meant
...
export interface HostInput {
   ...
   * `spec/interaction-events.md:184-185` (text that parses as an OpenLogo number literal reports a
   ...
   * other ending `spec/interaction-events.md:158-159` allows and the program is cancelled with
   ...
   * text (classified by `spec/interaction-events.md:184-185` exactly as a scripted answer is);
   * returning `undefined` means the host cannot or will not answer, which ends the read the only
   * other way `:158-159` allows — the program is cancelled.
   *
   * This is how a real host both **displays the prompt** (`:182`) and holds the read open: the
   * reader IS the outstanding read, so a caller can observe, from inside it, that no further
   * OpenLogo instruction and no event handler block has run — the normative MUST at `:156-159`,
 ...
 * learner (`spec/interaction-events.md:177`: the prompt MUST be a `word`) — and reports
 * the text the learner submitted, or `undefined` to leave the read unanswered and cancel the run.
 * Synchronous by design: `spec/interaction-events.md:156-159` requires that no OpenLogo instruction
