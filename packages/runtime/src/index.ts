 ...
 * per statement (`spec/execution-model.md#equality-and-ordering, spec/execution-model.md#numbers-and-math` — the `instruction` event is the unit of
 ...
 * `spec/execution-model.md#execution-safety`, `spec/error-model.md#normative-code-registry`). Every field is optional and
 ...
 * - `hostInput` (issue #686, slice I7 — `spec/interaction-events.md#time-ticks-and-handlers`) — a {@link HostInput}
 ...
 *   same-tick order (`when` → `on_key` → `on_click` → due `every`, `spec/interaction-events.md#time-ticks-and-handlers`)
 *   — from a pure headless `execute()` call with no real input device. The spec (`spec/interaction-events.md#time-ticks-and-handlers`) requires
 ...
 *   show the learner a question over a blank canvas. `spec/interaction-events.md#trace-stream-integration` explicitly
 ...
 *   carries one (`spec/interaction-events.md#time-ticks-and-handlers` makes a tick implementation-defined, and
 ...
 *   (`spec/interaction-events.md#trace-stream-integration`), never its key word, so answering it previously meant
...
export interface HostInput {
   ...
   * `spec/interaction-events.md#input-prompt-word` (text that parses as an OpenLogo number literal reports a
   ...
   * other ending `spec/interaction-events.md#trace-stream-integration` allows and the program is cancelled with
   ...
   * text (classified by `spec/interaction-events.md#input-prompt-word` exactly as a scripted answer is);
   * returning `undefined` means the host cannot or will not answer, which ends the read the only
   * other way `spec/interaction-events.md#trace-stream-integration` allows — the program is cancelled.
   *
   * This is how a real host both **displays the prompt** (`spec/interaction-events.md#input-prompt-word`) and holds the read open: the
   * reader IS the outstanding read, so a caller can observe, from inside it, that no further
   * OpenLogo instruction and no event handler block has run — the normative MUST at `spec/interaction-events.md#trace-stream-integration`,
 ...
 * learner (`spec/interaction-events.md#input-prompt-word`: the prompt MUST be a `word`) — and reports
 * the text the learner submitted, or `undefined` to leave the read unanswered and cancel the run.
 * Synchronous by design: `spec/interaction-events.md#trace-stream-integration` requires that no OpenLogo instruction
