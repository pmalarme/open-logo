 ...
 * per statement (`spec/execution-model.md:822-863` — the `instruction` event is the unit of
 ...
 * (with an optional `else`) and `while` their runtime meaning (`spec/execution-model.md:615-619`):
 ...
 * validates its count TYPE then RANGE, in that order (`spec/execution-model.md:615-619`) —
 ...
 * - `hostInput` (issue #686, slice I7 — `spec/interaction-events.md:137-139`) — a {@link HostInput}
 ...
 *   same-tick order (`when` → `on_key` → `on_click` → due `every`, `spec/interaction-events.md:130-135`)
 *   — from a pure headless `execute()` call with no real input device. The spec (`:137-139`) requires
 ...
 *   show the learner a question over a blank canvas. `spec/interaction-events.md:154-156` explicitly
 ...
 *   (`spec/interaction-events.md:166-168`), never its key word, so answering it previously meant
...
export interface HostInput {
   ...
   * `spec/interaction-events.md:182-183` (text that parses as an OpenLogo number literal reports a
   ...
   * other ending `spec/interaction-events.md:156-157` allows and the program is cancelled with
   ...
   * text (classified by `spec/interaction-events.md:182-183` exactly as a scripted answer is);
   * returning `undefined` means the host cannot or will not answer, which ends the read the only
   * other way `:156-157` allows — the program is cancelled.
   *
   * This is how a real host both **displays the prompt** (`:180`) and holds the read open: the
   * reader IS the outstanding read, so a caller can observe, from inside it, that no further
   * OpenLogo instruction and no event handler block has run — the normative MUST at `:154-157`,
 ...
 * learner (`spec/interaction-events.md:175`: the prompt MUST be a `word`) — and reports
 * the text the learner submitted, or `undefined` to leave the read unanswered and cancel the run.
 * Synchronous by design: `spec/interaction-events.md:154-157` requires that no OpenLogo instruction
