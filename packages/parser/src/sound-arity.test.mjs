 ...
 * `spec/tooling.md#layer-2-semantic-checking`), and `check()` must only recognize them as known callees when the
 ...
 * `spec/grammar.md#keywords-primitives-and-built-in-names` makes every profile's primitives built-in names unconditionally, so the
...
test("without the sound profile active, a wrong-arity Sound call is ol-unknown-command, not an arity error", () => {
  // Legality gating stays the checker's job (spec/tooling.md#layer-2-semantic-checking): with no `sound` profile the
...
test("a struct type name colliding with a Sound primitive raises ol-reserved-word", () => {
  for (const name of ["set_tempo", "beep", "note", "rest", "play"]) {
    ...
    // `params: { name }` only since issue #838 (`spec/error-model.md#normative-code-registry`).
...
test("a local naming a Sound primitive is a binding, so it raises nothing", () => {
  ...
  // slot, and `spec/grammar.md#keywords-primitives-and-built-in-names` makes accepting the name a MUST. The `define`/`struct` rows
...
test("#841: without the sound profile, define/struct set_tempo/beep/… still raise — only local stays free", () => {
  ...
  // `struct`) are profile-independent — `spec/grammar.md#keywords-primitives-and-built-in-names`, retired gate, issue #841 — while
  // `local` is a BINDING form that `spec/grammar.md#keywords-primitives-and-built-in-names` makes a MUST to accept, so it raises
