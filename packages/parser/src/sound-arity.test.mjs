 ...
 * `spec/grammar.md:412` makes every profile's primitives built-in names unconditionally, so the
...
test("a local naming a Sound primitive is a binding, so it raises nothing", () => {
  ...
  // slot, and `spec/grammar.md:390` makes accepting the name a MUST. The `define`/`struct` rows
...
test("#841: without the sound profile, define/struct set_tempo/beep/… still raise — only local stays free", () => {
  ...
  // `struct`) are profile-independent — `spec/grammar.md:412`, retired gate, issue #841 — while
  // `local` is a BINDING form that `spec/grammar.md:390` makes a MUST to accept, so it raises
