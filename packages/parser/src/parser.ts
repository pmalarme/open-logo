...
function collectUserArities(
  ...
  for (let k = 0; k < tokens.length; k += 1) {
    const head = tokens[k];
    if (head === undefined || head.kind !== "name") {
      continue;
    }
    if (head.text.toLowerCase() !== "define") {
      continue;
    }
    const nameTok = tokens[k + 1];
    if (nameTok === undefined || nameTok.kind !== "name") {
      continue;
    }
    let arity = 0;
    let j = k + 2;
    for (;;) {
      const param = tokens[j];
      if (param === undefined || param.kind !== "variable") {
        break;
      }
      arity += 1;
      j += 1;
...
export function parse(source: string, document = "<input>"): ParseResult {
  ...
  const eofToken: LexToken = tokens[tokens.length - 1] ?? {
    kind: "eof",
    text: "",
    source_span: makeSpan(document, [1, 1], [1, 1]),
  };

  let pos = 0;

  function current(): LexToken {
    return tokens[pos] ?? eofToken;
  }

  function peek(offset: number): LexToken {
    return tokens[pos + offset] ?? eofToken;
  }
  ...
  function previousEnd(): Position {
    const prev = tokens[pos - 1];
    return prev === undefined ? current().source_span.start : prev.source_span.end;
  }
  ...
  function spanToHere(start: Position): SourceSpan {
    return makeSpan(document, start, previousEnd());
  }
  ...
  function resync(): void {
    ...
    if (token.kind === "name" && token.text.toLowerCase() === "end") {
      diagnostics.push(parseDiag.mismatchedEnd(token.source_span, "block", "end"));
    } else if (token.kind === "name" && token.text.toLowerCase() === "else") {
      diagnostics.push(parseDiag.mismatchedEnd(token.source_span, "if", "else"));
    } else {
  ...
  function parseComparison(): ExpressionNode | undefined {
    ...
    const comparisons: ExpressionNode[] = [];
    for (let k = 0; k < ops.length; k += 1) {
      ...
      comparisons.push(ast.call(op, [left, right], spanBetween(left, right)));
    }
    let result = comparisons[0];
    if (result === undefined) {
      return first;
    }
    for (let k = 1; k < comparisons.length; k += 1) {
      const next = comparisons[k];
      if (next === undefined) {
        continue;
      }
      result = ast.call("and", [result, next], spanBetween(result, next));
    }
    return result;
  }
  ...
  function parseUnary(): ExpressionNode | undefined {
    ...
    if (token.kind === "op" && token.text === "-" && peek(1).kind === "number") {
      advance();
  ...
  function parseControlBody(opener: string, headerSpan: SourceSpan): BlockNode | undefined {
    const token = current();
  ...
  function parseColonAssignment(): StatementNode | undefined {
    ...
    return ast.assign(place, value, "equals", spanFrom(varTok.source_span.start, value));
  }
  ...
  function parseSetAssignment(): StatementNode | undefined {
    ...
    return ast.assign(place, value, "set", spanFrom(setTok.source_span.start, value));
  }
  ...
  function parseReturn(): StatementNode | undefined {
    ...
    return ast.returnStmt("return", value, spanFrom(token.source_span.start, value));
  }
