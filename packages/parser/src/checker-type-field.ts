 ...
 * call argument, and `spec/tooling.md:201-203` requires tools MUST NOT report speculative type
 ...
 * unknown field is reported at `check()` time (`spec/tooling.md:196`: a tool SHOULD report a
 ...
 * exactly the speculative inference `spec/tooling.md:199-203` forbids when a value is only known
...
export function resolveRecordField(
  ...
  // (`spec/error-model.md:256-261`). Report the folded field name (its case-insensitive resolution
 ...
 *
 * Scope is deliberately narrow (`spec/tooling.md:199` forbids speculative inference):
