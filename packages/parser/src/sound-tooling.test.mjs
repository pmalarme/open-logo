...
//      layers `defaultLibrary` on it (`spec/tooling.md:281-283`), exactly as a Core command like
...
test("semanticTokens: every Sound command carries defaultLibrary when nested in a whole program", () => {
  ...
  // must still surface as `primitive` + `defaultLibrary` semantic tokens (spec/tooling.md:281-283).
