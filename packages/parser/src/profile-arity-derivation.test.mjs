...
test("a Heritage alias is arity-checked as the canonical command it spells", () => {
  ...
  // (`spec/conformance.md:146`, `spec/error-model.md:255-260`).
...
test("params.callable is the canonical lowercase name for every profile, not the surface spelling", () => {
  // `spec/error-model.md:255-260` — diagnostic identity is `code` plus `params`, and the same
  // condition must carry the same params; `:200` prefers the canonical lowercase spelling for
  ...
  // "mention wrapping the call in `( … )`" — but not the exact English, and `:262-264` positively
...
test("a user procedure's and a struct constructor's params.callable is the DECLARED spelling, not the call site's", () => {
  ...
  // condition, which `spec/error-model.md:255-260` requires to carry one set of params. The call
