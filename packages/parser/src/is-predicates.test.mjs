// Unit tests for worded is-predicates (issue #53), per spec/grammar.md#expressions-and-calls and
// spec/execution-model.md#precedence-and-evaluation-order. `coverage.test.mjs` already exercises the four predicate forms'
...
// follow `is` (spec/grammar.md#expressions-and-calls; `of` is additionally structural in the Heritage
// `value of … for key` reader, spec/grammar.md#keywords-primitives-and-built-in-names, which is out of scope here), precedence against
// `and`/`or`
...
test("the contextual keywords `empty`, `member`, `of`, and `a` are ordinary names in plain read/call position (spec/grammar.md#expressions-and-calls)", () => {
  ...
  // (spec/grammar.md#keywords-primitives-and-built-in-names), which `heritage-tooling.test.mjs` and `value-of-key.test.mjs` cover.
