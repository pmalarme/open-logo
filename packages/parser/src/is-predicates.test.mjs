// Unit tests for worded is-predicates (issue #53), per spec/grammar.md:187-190,236 and
...
// follow `is` (spec/grammar.md:236; `of` is additionally structural in the Heritage
// `value of … for key` reader, spec/grammar.md:382, which is out of scope here), precedence against
// `and`/`or`
...
test("the contextual keywords `empty`, `member`, `of`, and `a` are ordinary names in plain read/call position (spec/grammar.md:236)", () => {
  ...
  // (spec/grammar.md:382), which `heritage-tooling.test.mjs` and `value-of-key.test.mjs` cover.
