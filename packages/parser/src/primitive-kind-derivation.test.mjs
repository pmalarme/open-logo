 ...
 * `repeat 4 [ forward 100 right 90 ]` — `spec/grammar.md#blocks-and-bracket-roles`, the specification's own
 ...
 * measured exactly that gap: flipping `stamp` to `reporter`, contradicting `spec/commands.md#stamp`,
...
test("a Heritage alias is classified as the canonical it is a spelling of", () => {
  ...
  // spellings only, no new semantics" (`spec/conformance.md#heritage`), so an alias must not carry a
...
test("the Data, Interaction & Events and Educational profiles are classified too (issue #932)", () => {
  // `wait`/`input` are the sharpest pair in the registry: one profile, one command, one reporter,
  // so swapping them is invisible to any sweep that asks the implementation what they are. Tutor's
  // `challenge` is pinned by the double-report test below, which needs its own profile set.
  const profiles = [
    "core-language",
    "data",
    "interaction-events",
...
test("challenge is classified a command even though it has no checker visibility yet (issue #932)", () => {
  ...
  // visible? does this statement produce a value?), and `spec/tooling.md#layer-2-semantic-checking` gives them
  // separate rows.
  assert.deepEqual(
...
test("the spec's own instruction-block example is style-clean (issue #932)", () => {
  // `repeat 4 [ forward 100 right 90 ]` is the example in `spec/grammar.md#blocks-and-bracket-roles`.
