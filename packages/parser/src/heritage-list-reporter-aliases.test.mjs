...
// `params`, and those params are CANONICAL, never the surface spelling (`spec/error-model.md:256-261`,
...
test("a parenthesized reporter alias raises the SAME arity diagnostic as its Core twin, with canonical structured params", () => {
  ...
  for (const [aliasSource, coreSource, canonical] of cases) {
    ...
    // assert on, per `spec/error-model.md:256-261`), `stage`, and `severity` all match, and the
