...
//                          any name."
//   spec/grammar.md:388 — every binding form "MUST accept **any** name, including a keyword, a
...
//   spec/grammar.md:390 — why the declaration slots are the complete enforcement point: "A
...
/**
 * Every binding position `spec/grammar.md:388` names, as a template that puts `name` in it. `make`
...
function assertBindingIsFree(form, word, profiles) {
  ...
  assert.deepEqual(
    ...
    [],
    `${form.label} with \`${word}\` must be accepted (spec/grammar.md:388)`,
  );
}
...
test("every binding form is free for a profile word — in BOTH directions", () => {
  // `spec/grammar.md:410` makes profile words built-in unconditionally, and binding a built-in name
...
test("`local` is a binding, not a declaration: it registers nothing callable", () => {
  // This is the evidence `spec/grammar.md:390` rests on, asserted rather than quoted. `local foo`
...
test("a nested place is a write into an existing structure, so it stays clean", () => {
  // spec/grammar.md:406,408 — "A write **into** an existing value introduces no name … a postfix
