...
//
//   spec/grammar.md#keywords-primitives-and-built-in-names — "A program may not declare a built-in name. A program may bind a value to
//                          any name."
//   spec/grammar.md#keywords-primitives-and-built-in-names — every binding form "MUST accept **any** name, including a keyword, a
...
//   spec/grammar.md#keywords-primitives-and-built-in-names — why the declaration slots are the complete enforcement point: "A
...
/**
 * Every binding position `spec/grammar.md#keywords-primitives-and-built-in-names` names, as a template that puts `name` in it. `make`
...
function assertBindingIsFree(form, word, profiles) {
  ...
  assert.deepEqual(
    ...
    [],
    `${form.label} with \`${word}\` must be accepted (spec/grammar.md#keywords-primitives-and-built-in-names)`,
  );
}
...
test("every binding form is free for a profile word — in BOTH directions", () => {
  // `spec/grammar.md#keywords-primitives-and-built-in-names` makes profile words built-in unconditionally, and binding a built-in name
...
test("`local` is a binding, not a declaration: it registers nothing callable", () => {
  // This is the evidence `spec/grammar.md#keywords-primitives-and-built-in-names` rests on, asserted rather than quoted. `local foo`
...
test("a nested place is a write into an existing structure, so it stays clean", () => {
  // spec/grammar.md#keywords-primitives-and-built-in-names — "A write **into** an existing value introduces no name … a postfix
...
test("non-regression: `define` keeps its full check, now split across two codes", () => {
  ...
  // thing (`spec/error-model.md#normative-code-registry`): a keyword or primitive is `ol-reserved-word` ("OpenLogo
...
test("call sites stay legal — the rule is keyed to `declared-callable-name`, not `callable-name`", () => {
  // spec/grammar.md#ebnf-notation — `callable-name`/`type-name` remain the CALL slots, "where every built-in
