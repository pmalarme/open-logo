...
// (`spec/grammar.md#keywords-primitives-and-built-in-names`) — so, apart from the handful the `expression`
// production itself admits (`spec/grammar.md#expressions-and-calls`), none of them may be read as a bare call.
//
 ...
 * sweep must exempt: `true`/`false` are the `boolean-literal` production (`spec/grammar.md#expressions-and-calls`),
 * `map`/`filter`/`reduce` open a `comprehension` (`spec/grammar.md#ebnf-notation`), and `thing` is the one
...
test("a keyword with no expression-head role reports ol-bad-token naming it", () => {
  for (const word of OL.OL_KEYWORDS) {
    if (EXPRESSION_INITIAL.has(word) || word === "not") {
      // `not` is consumed as the prefix operator (`spec/grammar.md#expressions-and-calls`) before the reader reaches
...
test("no keyword except `thing`, `and`, and `or` is a parenthesized-call callee", () => {
  // `parenthesized-call ::= "(" callable-name { expression } ")"` (`spec/grammar.md#expressions-and-calls`). Three
  ...
  //   normative: "`(and …)` for multiple operands" (`spec/commands.md#and`, `spec/commands.md#or`) with
  //   short-circuit semantics (`spec/execution-model.md#precedence-and-evaluation-order`).
  //
...
test("`( not 1 )` stays a grouped unary, never a parenthesized call", () => {
  // `unary ::= "not" unary` (`spec/grammar.md#expressions-and-calls`) inside a `parenthesized-expression`
  // (`spec/grammar.md#expressions-and-calls`) — parenthesising groups the unary expression, it does not make `not` a
  // `callable-name`, and unlike `and`/`or` the spec gives `not` no paren form at all
  // (`spec/commands.md#not`: "Signature: `not boolean`", "Kind: Reporter unary prefix").
...
test("keywords are still legal data — dict keys, fields, and bare selector keys", () => {
  ...
  // keys" (`spec/grammar.md#keywords-primitives-and-built-in-names`). `[key]` is the BARE `identifier` alternative of `key-term`
  // (`spec/grammar.md#ebnf-notation`) — the case that actually routes through `parseKeyTerm`'s `name` branch,
  ...
  for (const source of sources) {
    assert.deepEqual(
      OL.parse(source, doc).diagnostics,
      [],
      `\`${source}\` uses a keyword as data, which stays legal (spec/grammar.md#keywords-primitives-and-built-in-names)`,
    );
  }
});
...
test("a keyword as a bare place still parses, and binding it is legal", () => {
  ...
  // implemented for the parser by #837) reverses it. `spec/grammar.md#keywords-primitives-and-built-in-names` now makes accepting any
  ...
  // (`spec/grammar.md#keywords-primitives-and-built-in-names`). So both halves are clean, and the reader half is unchanged either way.
