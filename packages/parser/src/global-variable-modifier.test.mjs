...
test("a procedure written inside a block is sealed from that block's locals", () => {
  ...
  // bindings, and the globals. A `define` inside a block is reachable (`:651` registers it
  // globally like any other), so the seal has to hold there too, and this is the case that proves
  // the walk RESTARTS rather than inheriting the enclosing shadows (issue #826 review, round 1,
  // @testing finding 3 — the surviving mutant M6b).
  const source = [
    "global x = 0",
    "repeat 2 [",
    "  local x = 1",
...
// --- The spellings that carry no signal, and why ------------------------------

test("a `set` target and a `make` target carry no signal — they are not `:variable`", () => {
  // Not an omission: `spec/grammar.md:107-108` makes `set`'s target a BARE place and `make`'s a
  // word literal, and `spec/tooling.md:34` gives the `:variable` class to a COLON-FORM place head
  // only, while `:25-26` is a MUST NOT against classifying anything inside a closed string as a
  // variable. Painting either would mean inventing a class the normative table does not assign.
  // Issue #1107 asks the maintainer to rule on bare place heads and binder spellings as one
  // question (issue #826 review, round 1, rubber-duck finding 2).
  const source = [
    "global shared = 0",
    "set shared to 1",
    'make "shared" 2',
  ...
  assert.deepEqual(variableFlags(source), []);
  assert.deepEqual(globalModifierTexts(source), []);
  // They are painted, just not as variables — so nothing here is unclassified.
