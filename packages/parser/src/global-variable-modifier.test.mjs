...
test("a name the program only DECLARES is not marked", () => {
  ...
  // declared name is excluded because it *introduces* a binding rather than resolving one, which is
  // the same reason a procedure's `:param` binding site is not a use either — though that one still
  // reads `false` rather than nothing, because the field is total on `:variable`. Whether a declared
  // name should also stop reading as a library call (it still takes `defaultLibrary`, #831's
  // over-application) is issue #831's question, not this slice's.
...
test("a token the resolver reported no use for carries no `global` field", () => {
  // The distinction that makes the field readable: absent means "no use reported here", which is
  // different from `false`, "a use that is not shared". Absent is NOT "names no variable" — a bare
  // binding site names one and is still absent, which the declaration test below pins.
...
test("`global` is total on `:variable`, and present elsewhere only for a resolver-reported use", () => {
  const source = [
    "global count = 0",
    'print "hello"',
    "set count to 1",
