...
// of `spec/grammar.md:392`: "A keyword in a position none of these cover has no derivation at all
...
// set (`spec/grammar.md:410`). This rule still asks its own question with the profile set, because
...
test("every registry word is deliberately classified Kind S or Kind C — no default", () => {
  // The drift guard for the rule's `SPECIAL_FORM_PROFILE_WORDS`. `spec/grammar.md:392` matches a
...
test("every special-form word is rejected in value position when its profile is active", () => {
  ...
  // the `repeat` count from `spec/grammar.md:392`'s own worked example.
...
test("`tell` is a COMMAND, not a special form — `( tell :t )` stays legal", () => {
  ...
  // table: "The C3 Sprites rows are authoritative"), and `spec/grammar.md:410` calls it "the Sprites
  // command `tell` — a mode switch that takes no block". A command HAS a callable form, so
  // `spec/grammar.md:392` matches `tell` as a `callable-name` and `( tell :t )` is a legitimate
...
test("a profile word is rejected as a parenthesized-call callee too, in either position", () => {
  ...
  // `spec/grammar.md:392` matches a keyword as `callable-name` "only where the C3 primitive matrix
...
test("without its profile, a profile word stays an ordinary name in VALUE position — no ol-bad-token", () => {
  ...
  // `spec/grammar.md:410` moved one of them and not the other.
...
test("profile words stay legal as data and in every binding position", () => {
  // `spec/grammar.md:388` makes accepting any name in a binding position a normative MUST, and
  // `:408` makes dictionary keys and bare selector keys data. None of these lowers the word into a
