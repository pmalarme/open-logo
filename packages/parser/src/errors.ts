 ...
 * in a position none of `spec/grammar.md:394`'s name-admitting positions cover *"has no derivation at
...
export const parseDiag = {
   ...
   *   in **Data**; `spec/grammar.md:394` says a bare `value` heads the heritage reader *"where
   ...
   * the six** probed positions, not all six. `spec/grammar.md:394` guarantees only the weaker
   ...
   * wrong for this code, because `spec/grammar.md:390` makes binding a keyword legal — `local set` and
