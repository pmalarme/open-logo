...
export const parseDiag = {
  ...
  unmatchedBracket(
    ...
    return parseError(
      ...
      span,
      openedAt === undefined ? { delimiter } : { delimiter, opened_at: openedAt },
  ...
  unmatchedBrace(
    ...
    return parseError(
      ...
      span,
      openedAt === undefined ? { delimiter } : { delimiter, opened_at: openedAt },
      `this ${delimiter} doesn't have a matching brace. a dictionary needs both { and }.`,
    );
  },
  ...
  missingEnd(span: SourceSpan, opener: string, hint?: string): Diagnostic {
    const repair = hint ?? "wrap the body in [ ] or close it with end.";
    return parseError(
      "ol-missing-end",
      span,
      { opener, hint: repair },
      `${opener} needs a body. ${repair}`,
    );
  },

  mismatchedEnd(span: SourceSpan, expected: string, actual: string): Diagnostic {
    return parseError(
      "ol-mismatched-end",
      span,
      { expected, actual },
