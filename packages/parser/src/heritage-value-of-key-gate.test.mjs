...
// (spec/grammar.md#expressions-and-calls) is a Heritage spelling of the Core dict read `:d[:k]`/`:d.key`
// (spec/conformance.md#feature-to-profile-table), and because it operates on a dict it also needs the Data profile
// (spec/conformance.md#profile-dependency-dag). Parsing of the form itself (into a `ValueOfKey` node) landed with
...
// Heritage depends on Data (spec/conformance.md#profile-dependency-dag), so the "active" set claims both. Data also
