...
test("ol-style-name-case: every silent row of issue #854's reported table now warns", () => {
  ...
  // the bug: `spec/tooling.md#layer-3-style-lints` requires that "built-ins should be shown lowercase", and
...
test("ol-style-name-case: a built-in keeps its identity even when a program illegally declares it", () => {
  // `spec/grammar.md#keywords-primitives-and-built-in-names` is "a program may not declare a built-in name", so `define print … end`
...
test("ol-style-name-case: a bare word-literal key is data, never a miscased built-in", () => {
  // Dictionary and selector keys may be written bare, and `spec/grammar.md#keywords-primitives-and-built-in-names` makes a keyword
...
test("ol-style-name-case: built-in casing is judged the same under any profile set", () => {
  // A built-in name's *identity* is profile-independent (`spec/grammar.md#keywords-primitives-and-built-in-names`: a program cannot
...
// (issue #1025). This wording deliberately does not opt in. `spec/error-model.md#localization-boundary` makes
// identity `code` plus `params` and asks tests to assert those, and `spec/error-model.md#localization-boundary` positively permits a
