...
//      their DECLARATION is not: `spec/grammar.md#keywords-primitives-and-built-in-names` makes profile words built-in names
...
//      whether it is reportable at all (`spec/tooling.md#layer-2-semantic-checking`), so `define who` collides under
...
// active-profile set. `spec/tooling.md#normative-token-class-model` puts in the `keyword` class, "while their profile is
// active", "a profile's block-heads and its mode-switch commands" — so precisely: `ask` and `each`
// are the block-heads and `tell` is the
// mode-switch command, and all three move. `spec/tooling.md#normative-token-class-model` puts "a profile word whose profile is inactive"
...
// either way — they are ordinary primitives (`spec/tooling.md#normative-token-class-model`, "profile primitives when enabled"), which is
...
// predates #740 and is left as-is; `spec/tooling.md#normative-token-class-model` is the precise wording.)
//
...
test("highlight: a profile word in an ORDINARY-NAME position still follows the profile", () => {
  ...
  // `dict-key` "on grammatical grounds alone" (`spec/tooling.md#normative-token-class-model`), so it must NOT follow the profile. Pinning
...
test("check: redefining a Sprites block-head under an active profile raises ol-reserved-word", () => {
  ...
  // `spec/grammar.md#keywords-primitives-and-built-in-names` overruled and issue #841 removed — so this test's Core-only twin below
...
test("#841: redefining a Sprites block-head raises under Core-only too", () => {
  // `spec/grammar.md#keywords-primitives-and-built-in-names` and `spec/turtles-and-sprites.md#reserved-words-in-this-profile`: a profile decides whether a name
...
test("check: redefining a Sprites reporter under an active profile raises ol-reserved-word", () => {
  ...
  // pinned the defect rather than the rule. `spec/tooling.md#layer-2-semantic-checking` is a normative Layer-2 "Required
  ...
  // `namespace` param, because `spec/error-model.md#normative-code-registry` makes keyword-vs-primitive "an
...
test("#841: redefining a Sprites reporter raises under Core-only too", () => {
  ...
  // (`spec/tooling.md#layer-2-semantic-checking`) and a different rule, exercised above.
  for (const reporter of Object.keys(SPRITES_REPORTERS)) {
