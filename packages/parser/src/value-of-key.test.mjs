...
// `spec/grammar.md#expressions-and-calls`'s `value-of-reader`). `coverage.test.mjs`'s `MEGA` walk exercises the
...
test("a bare `value` not followed by `of` is rejected as a misplaced reserved word, not read as a call", () => {
  ...
  // `value` is globally reserved (`spec/grammar.md#keywords-primitives-and-built-in-names`) and heads no `expression` alternative but
  // the `of`-gated reader above, so outside that form it is not permitted at this grammar position
  // (`spec/error-model.md#normative-code-registry`). Before issue #853 it fell through to a bare zero-argument call
