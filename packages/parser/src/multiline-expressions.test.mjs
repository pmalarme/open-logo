// Issue #933 — `spec/grammar.md#lexical-form-and-encoding`: "Within a single expression, list literal, dict literal, or
...
// genuinely ambiguous. `spec/grammar.md#collections-records-and-comprehensions`'s **Entry lookahead** rule settles it by the token that
...
test("#944: the one-line word-key spellings now parse as two entries", () => {
  ...
  // entries, so a newline inside a dict literal was significant, against `spec/grammar.md#lexical-form-and-encoding`.
...
// `parenthesized-call` (`==` is not an `expression`, spec/grammar.md#expressions-and-calls) and exactly one as a
// `parenthesized-expression` (spec/grammar.md#expressions-and-calls). It was a reader defect all along, and needed no spec change.
//
...
test("#709/#1021: every binary operator is admitted, `-` included", () => {
  // All of `spec/grammar.md#expressions-and-calls`'s infix operators: the ten symbolic ones plus the worded
  ...
  // adjacency (`spec/grammar.md#ebnf-notation, spec/grammar.md#expressions-and-calls`).
...
test("issue #709 reads a line-spanning group exactly as its one-line form", () => {
  // `spec/grammar.md#lexical-form-and-encoding` — newlines are insignificant inside one parenthesized group — so the
...
// (`spec/grammar.md#expressions-and-calls`, `postfix-expression ::= primary { selector | "." identifier }`), and the
...
test("#1025: a group headed by a callable with a `.field` postfix reads exactly like its bare spelling", () => {
  for (const [parenthesized, bare] of [
    ...
    ["print (origin . x)", "print origin . x"],
    // And across a newline, which `spec/grammar.md#lexical-form-and-encoding` makes insignificant inside one group and
...
test("#1025: a SPACED `[` after the head keeps the parenthesized call — adjacency is the discriminator", () => {
  ...
  // `( pair [1] )` is still `parenthesized-call` (`spec/grammar.md#expressions-and-calls`) passing a one-element list
...
test("#1025: the postfix lookahead is NOT asked of the `and`/`or` heads, which are no primaries", () => {
  ...
  // callable (`spec/grammar.md#keywords-primitives-and-built-in-names`) — they answer false to `isCalleeName` and can never be the
...
test("#1021: a head short of an input keeps the branch's pre-existing recovery", () => {
  ...
  // `spec/error-model.md#normative-code-registry` is the MUST NOT that governs every recovery path: no
...
test("#1021: the arity-short RECOVERY still reads the two spellings apart (saga #1017)", () => {
  ...
  // The branch DECISION itself is newline-blind, which is what `spec/grammar.md#lexical-form-and-encoding` requires: an
