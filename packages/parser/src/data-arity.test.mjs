 ...
 * F3, `spec/tooling.md#layer-1-lex-and-parse-checking, spec/tooling.md#layer-2-semantic-checking`). `dict`/`keys`/`values`/`type_of`/`reverse`/`pick`/`sort` and a
 ...
 * genuinely profile-scoped — `spec/tooling.md#layer-2-semantic-checking` gates which names are *available*, so
 * without `data` a Data primitive is an unknown callee like any other undeclared name, and its
 * arity is nobody's business. **Reserved-word collision is not.** `spec/grammar.md#keywords-primitives-and-built-in-names` makes the
...
// theoretical.
//
// `spec/data-structures.md#records-and-structs` describes registration as ONE act: a `struct` declaration registers
...
// `spec/execution-model.md#reader-pipeline` answers it without one), while `collectVisibleNames` gates the
...
// `ol-unknown-command` axis (is a name AVAILABLE), which `spec/tooling.md#layer-2-semantic-checking` genuinely does
...
// `spec/data-structures.md#records-and-structs` requires when `data` is not claimed — so it needs a maintainer, and
...
test("a struct type name colliding with a Data primitive raises ol-reserved-word", () => {
  ...
  // this name" and carries `params: { name }` only (`spec/error-model.md#normative-code-registry`).
...
test("a struct type name colliding with a define'd procedure raises ol-duplicate-definition", () => {
  ...
  // `ol-reserved-word` (`spec/grammar.md#keywords-primitives-and-built-in-names`, `spec/error-model.md#normative-code-registry`) and gave it both spans.
...
test("without the data profile active, a struct DUPLICATING a procedure is still reported", () => {
  ...
  // — and `spec/execution-model.md#reader-pipeline` answers it with no profile condition: "The reader
  ...
  // `ol-duplicate-definition`". `spec/data-structures.md#records-and-structs` says the same. The runtime's phase-1
