 ...
 * names**, governed by one rule (`spec/grammar.md#keywords-primitives-and-built-in-names`):
 *
 ...
 *   dictionary key (`spec/grammar.md#keywords-primitives-and-built-in-names`, a normative MUST). The rule is enforced at the four
 ...
 *   on purpose" (`spec/grammar.md#keywords-primitives-and-built-in-names`). Reserved-list membership and token class are independent
 *   axes.
 *
 ...
 * `value of :d for key "a"` still reports the key's value afterwards (`spec/grammar.md#keywords-primitives-and-built-in-names`). Their
 * *highlighting* is positional too: `spec/tooling.md#reserved-words-for-tooling` marks them `keyword` only inside an
...
/**
 * The keywords, in the grammar's grouping order (`spec/grammar.md#keywords-primitives-and-built-in-names`).
 *
 ...
 * one:
 *
 * - `spec/grammar.md#keywords-primitives-and-built-in-names` — normative; `npm run built-in-names` compares its extracted words
 *   against `spec/built-in-names.json`.
 * - `spec/tooling.md#reserved-words-for-tooling` — mirrors that block; the same gate compares **the same extracted words
 ...
 * - `spec/tooling.md#normative-token-class-model` — the `keyword` **token class**, a different set on purpose
 *   (`spec/grammar.md#keywords-primitives-and-built-in-names`). Since issue #959 the row no longer enumerates it: each name's class is
 ...
 *
 * - **May a program declare this name?** No. `spec/grammar.md#keywords-primitives-and-built-in-names` makes profile words built-in
 ...
 *   `spec/grammar.md#keywords-primitives-and-built-in-names` makes accepting any of these words as a **binding** a MUST, so
 ...
 *   `spec/tooling.md#normative-token-class-model`'s `keyword` row states directly ("Profile words … take this class while
 ...
 * folding it in would make `isKeyword`'s profile gate — the paint axis `spec/tooling.md#normative-token-class-model` asks
 ...
 * a word that fails this test still may not be **declared**, because `spec/grammar.md#keywords-primitives-and-built-in-names` makes
 ...
 * **declaration** axis of `spec/grammar.md#keywords-primitives-and-built-in-names` — "what a profile decides is whether a name
 ...
 * counts. Keep the profile-gated {@link isKeyword} for the paint axis, where `spec/tooling.md#normative-token-class-model`
 * does ask for a gate.
 *
 * "Declare" is exact: this is the four declaration slots of `spec/grammar.md#keywords-primitives-and-built-in-names`, not bindings.
 * `spec/grammar.md#keywords-primitives-and-built-in-names` makes accepting a keyword as a **binding** a MUST, so `local ask` stays
 ...
 * `spec/tooling.md#normative-token-class-model`'s "while their profile is active" clause is decided here rather than
 ...
 * {@link isKeywordInAnyProfile}, because `spec/grammar.md#keywords-primitives-and-built-in-names` makes that answer
 * profile-independent. None of them asks about **bindings**, which `spec/grammar.md#keywords-primitives-and-built-in-names` requires
