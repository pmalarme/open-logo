...
// per spec/grammar.md#ebnf-notation (`long-control-block ::= terminator { statement
...
test("a mismatched end label on a long block raises `ol-mismatched-end` at parse stage (per spec/grammar.md#blocks-and-bracket-roles)", () => {
...
// Per spec/grammar.md#ebnf-notation, a `for … in` binder may be a bare `name` OR a destructuring
// `[ :name { :name } ]` pattern (e.g. `for [:x :y] in :points`, spec/grammar.md#collections-records-and-comprehensions).
...
test("an empty destructuring binder `for [] in …` reports the empty binder exactly once", () => {
  // This pinned "exactly one `ol-unmatched-bracket`, no duplicate" until `spec/error-model.md#normative-code-registry`
