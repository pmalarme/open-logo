 ...
 * effect.
 *
 * The occurrences are every token that **names a variable**, which `spec/grammar.md:106-111` spells
 * four ways — and they are marked on whatever class each already carries, because a **modifier
 * decorates a class rather than replacing it**. That is the whole reason this works without a
 * `spec/` change, and an earlier revision of this slice got it wrong: it declined the worded forms
 * on the grounds that they are not `:variable` tokens, which is true and irrelevant.
 *
 ...
 *
 * A **declaration** carries nothing, and that is the one deliberate exclusion: `global count = 0`
 * and `local count` write their names bare, but those tokens *introduce* a binding rather than
 * resolve to one, exactly as a procedure's `:param` binding site does (which is why the parameter's
 * own token reads `false`). Everything else is reached by the generic descent, which is why a
 * `Global`/`Local` node needs no arm of its own: its one walkable child is the initializer, an
 * ordinary read position, and its declared name is `SpannedName` metadata rather than a node.
 *
...
/**
 * One token that **names a variable**, and whether that name resolves to a `global` binding.
 ...
 * here. Every occurrence is recorded, not only the global ones, so a consumer can tell "a token
 * that names a variable, and it is not global" from "a token that names no variable at all".
 *
...
/**
 * Every token that **names a variable**, in source order, each with whether it resolves to a
 * `global` binding — see {@link VariableOccurrence} and the module doc comment.
 *
