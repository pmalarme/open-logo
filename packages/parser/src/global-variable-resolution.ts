 ...
 * binding site carries `false` rather than a resolution. (It carries `false` only because the field
 * is total on `:variable`; the bare ones carry nothing at all, which is the asymmetry
 * `Token.global`'s own contract spells out.) Everything else is reached by the generic descent,
 ...
 * tell "uses a variable, not shared" from "names no variable", on the most common document shape
