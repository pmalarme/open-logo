 ...
 *
 * Two related things are intentionally out of scope. A `global`/`local` declaration writes its name
 * **bare**, so `count` in `global count = 0` is a `name` token that `spec/tooling.md:31`'s
 * grammar-safe fallback paints `primitive`, exactly as `local count = 0` already did; changing that
 * is a question about *binder* spellings in general, not about globals. And `thing "count"` reads a
 * variable through a `word/string` literal, which is not a `:variable` token at all.
 *
 ...
 * is why this takes no enclosing shadow set at all.
 *
 * An optional parameter's default (`(:step 2)`, `spec/grammar.md:152`) is walked the same way. The
 * spec does not say which scope evaluates a default, so the paint takes the reading the boundary
 * already forces everywhere else — a name written inside the procedure's own header resolves the
 * way one written in its body does — which is also `checker-undefined-var.ts`'s reading, so the two
 * do not offer a learner two different answers about the same expression.
 */
function visitProcedureDef(
  ...
  for (const parameter of node.params) {
    parameters.add(parameter.name.name.toLowerCase());
  }
  for (const parameter of node.params) {
    if (parameter.defaultValue !== undefined) {
      visitNode(parameter.defaultValue, parameters, resolution);
    }
  }
  visitStatements(node.body.body, parameters, false, resolution);
}
