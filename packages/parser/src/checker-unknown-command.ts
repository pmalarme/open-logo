 ...
 * Levenshtein did-you-mean suggestion per `spec/error-model.md#normative-code-registry` /
 * `spec/tooling.md#layer-2-semantic-checking`.
 */
 ...
 * the "full canonical names over short aliases" step of `spec/error-model.md#normative-code-registry`.
 */
 ...
 * callee (`spec/grammar.md#expressions-and-calls`, and `signatures.ts`'s file doc comment). These come from
 ...
 * visible, so no diagnostic fires (`spec/tooling.md#layer-2-semantic-checking`).
 */
 ...
 * within {@link MAX_SUGGESTION_DISTANCE}. Deterministic tie-break per `spec/error-model.md#normative-code-registry`:
 ...
 * same Levenshtein distance. Three ordered rungs, per `spec/error-model.md#normative-code-registry`:
 *
...
/** The learner-facing message template from `spec/error-model.md#normative-code-registry`. */
...
export function unknownCommandRule(
  ...
  walk(program, (node) => {
    ...
    // diagnostic's identity (`spec/error-model.md#localization-boundary`): `Mystery`, `MYSTERY`, and `mystery`
