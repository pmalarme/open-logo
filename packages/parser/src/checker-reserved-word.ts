 ...
 * `ol-reserved-word`. #833 overrules that: `spec/grammar.md:388` now requires that an
 ...
 * (`spec/grammar.md:390`): "A restriction on a binding form is bypassable, because `local` is
 ...
 * (`spec/grammar.md:412`) — `alias definir define` is legal, which is how a localized keyword pack
 ...
 * in the callable namespace at all: `spec/grammar.md:408` — "Record field names live in a per-type
 ...
 *   `procedure` and `struct` namespaces move here (`spec/grammar.md:414`), and the code carries
 ...
 * `spec/grammar.md:416` makes the set "exactly the keywords listed above plus every primitive …
 * and every alias spelling", and `:410` makes it profile-independent, so this rule asks one
 ...
 * key — is deliberately not visited: `spec/grammar.md:388` makes accepting them a MUST.
 *
 ...
 * (`spec/grammar.md:410`). This rule answers "may the program declare it", which a profile never
