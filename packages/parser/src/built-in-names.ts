 ...
 *
 * `spec/grammar.md#keywords-primitives-and-built-in-names` defines the set as "exactly the keywords listed above plus every primitive
 ...
 * slots of `spec/grammar.md#keywords-primitives-and-built-in-names` (`define`, the heritage `to`, `struct`, and the first operand of
 * `alias`) and at no other position. `spec/grammar.md#keywords-primitives-and-built-in-names` makes accepting a built-in name as a
 ...
 * (`spec/conformance.md#heritage`), so `define pr` must be exactly as illegal as `define print`, and
 ...
 * would be invisible and unpredictable to a learner" (`spec/grammar.md#keywords-primitives-and-built-in-names`). **Do not add one:**
 * a profile-gated answer here makes `define ask` legal for a Core-only program and illegal for a
 * Sprites one, which is exactly the implementation-dependent outcome `spec/grammar.md#keywords-primitives-and-built-in-names` forbids.
 *
 ...
 * (`spec/tooling.md#layer-2-semantic-checking`, `checker-arity.ts`'s `activeProfilePrimitiveArityRange`). Declaring
