 ...
 *
 * `spec/grammar.md:416` defines the set as "exactly the keywords listed above plus every primitive
 ...
 * `alias`) and at no other position. `spec/grammar.md:388` makes accepting a built-in name as a
 ...
 * would be invisible and unpredictable to a learner" (`spec/grammar.md:410`). **Do not add one:**
 * a profile-gated answer here makes `define ask` legal for a Core-only program and illegal for a
 * Sprites one, which is exactly the implementation-dependent outcome `:410` forbids.
 *
