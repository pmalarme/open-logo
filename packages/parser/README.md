...
open — "optional modifiers **such as** …". The normative class table is unchanged; see
...
behind it, and names the two cases it knowingly does not reach: a `set`/`make` assignment target,
whose token is not `:variable` at all (issue #1107), and a deferred handler body, which one static
class cannot decide (issue #1108).
