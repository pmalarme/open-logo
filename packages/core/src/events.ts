...
/**
 * The registered kinds whose envelope **may carry** a `turtle_id`. `spec/execution-model.md:1003` is
 ...
 * default applied when the caller omitted them (`spec/execution-model.md:1140-1178`'s worked
 ...
 * (`spec/execution-model.md:1140-1178`, e.g. `{name:"countdown", result:0}`). `result` is `null`
 * when the invocation is a command — it finished (or `stop`ped) without reaching `return`
 * (`spec/execution-model.md:709-726`) — rather than `0`/`false`/an empty list, which are
 ...
 * (`spec/execution-model.md:1140-1178`, e.g. `{value:0}`). Emitted only when a procedure actually
 ...
 * (`spec/execution-model.md:1005-1017`). Present in {@link TutorOutputPayload} only when
 * `command` is `"hint"`.
 */
 ...
 * (`spec/execution-model.md:1068`) — so it is profile-neutral and the set of emitters is
 ...
 * `primitive`", `spec/interaction-events.md:165-166`; "wait emits a `primitive` event after the
 * pause completes … event registration forms emit `primitive` events after the handler is
 * registered", `spec/interaction-events.md:180-182`), but the type deliberately does not close over
 * them.
 */
 ...
 * "present only when the event is turtle-specific" (`spec/execution-model.md:1003`): addressing
 ...
 * without a more specific event (`spec/execution-model.md:1068`) — profile-neutral, not scoped to
 ...
 * ("primitives without a more specific kind emit `primitive`", `spec/interaction-events.md:165-166`).
 * It is deliberately NOT a new registered `kind`: the registry's `kind` values are normative and
 * closed (`spec/execution-model.md:1054-1059`, "One registered event kind"), the only sanctioned
 ...
 * catch-all (`spec/execution-model.md:1068`): its `name` must stay an OPEN type so a future primitive
 ...
 * `spec/interaction-events.md:346-359`): the tempo, in beats per minute, that `set_tempo` set.
 * Durations elsewhere in the stream are carried in beats and interpreted at the current tempo
 * (`spec/interaction-events.md:354-355`). A positive number (`ol-range` otherwise);
 ...
 * `spec/interaction-events.md:361-378`): one pitched sound scheduled at the current tempo. `pitch`
 ...
 * its positive beat `duration` (`spec/interaction-events.md:380-394` — the melody list is
 ...
 * `spec/interaction-events.md:380-394`): the resolved melody, as an ordered list of pitch/duration
 ...
 * `spec/interaction-events.md:396-411`): one short, implementation-defined alert sound. It carries
 ...
 * `spec/interaction-events.md:413-428`): scheduled silence of `duration` beats at the current
 * tempo. `rest` emits a `sound` event "so replay tools can show the silent interval"
 * (`spec/interaction-events.md:422`). `duration` is a positive number.
 */
 ...
 * the sound state has been scheduled (`spec/interaction-events.md:180-181`); the payload carries
 ...
 * profile-neutral generic catch-all (`spec/execution-model.md:1068`, via {@link PrimitivePayload} —
