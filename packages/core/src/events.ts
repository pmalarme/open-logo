...
/**
 * The registered kinds whose envelope **may carry** a `turtle_id`. `spec/execution-model.md:998` is
 ...
 * default applied when the caller omitted them (`spec/execution-model.md:1135-1173`'s worked
 ...
 * (`spec/execution-model.md:1135-1173`, e.g. `{name:"countdown", result:0}`). `result` is `null`
 * when the invocation is a command — it finished (or `stop`ped) without reaching `return`
 * (`spec/execution-model.md:706-721`) — rather than `0`/`false`/an empty list, which are
 ...
 * (`spec/execution-model.md:1135-1173`, e.g. `{value:0}`). Emitted only when a procedure actually
 ...
 * (`spec/execution-model.md:1000-1012`). Present in {@link TutorOutputPayload} only when
 * `command` is `"hint"`.
 */
 ...
 * (`spec/execution-model.md:1063`) — so it is profile-neutral and the set of emitters is
 ...
 * `primitive`", `spec/interaction-events.md:162-163`; "wait emits a `primitive` event after the
 * pause completes … event registration forms emit `primitive` events after the handler is
 * registered", `spec/interaction-events.md:177-179`), but the type deliberately does not close over
 * them.
 */
 ...
 * "present only when the event is turtle-specific" (`spec/execution-model.md:998`): addressing
 ...
 * without a more specific event (`spec/execution-model.md:1063`) — profile-neutral, not scoped to
 ...
 * ("primitives without a more specific kind emit `primitive`", `spec/interaction-events.md:162-163`).
 * It is deliberately NOT a new registered `kind`: the registry's `kind` values are normative and
 * closed (`spec/execution-model.md:1049-1054`, "One registered event kind"), the only sanctioned
 ...
 * catch-all (`spec/execution-model.md:1063`): its `name` must stay an OPEN type so a future primitive
 ...
 * `spec/interaction-events.md:343-356`): the tempo, in beats per minute, that `set_tempo` set.
 * Durations elsewhere in the stream are carried in beats and interpreted at the current tempo
 * (`spec/interaction-events.md:351-352`). A positive number (`ol-range` otherwise);
 ...
 * `spec/interaction-events.md:358-375`): one pitched sound scheduled at the current tempo. `pitch`
 ...
 * its positive beat `duration` (`spec/interaction-events.md:377-391` — the melody list is
 ...
 * `spec/interaction-events.md:377-391`): the resolved melody, as an ordered list of pitch/duration
 ...
 * `spec/interaction-events.md:393-408`): one short, implementation-defined alert sound. It carries
 ...
 * `spec/interaction-events.md:410-425`): scheduled silence of `duration` beats at the current
 * tempo. `rest` emits a `sound` event "so replay tools can show the silent interval"
 * (`spec/interaction-events.md:419`). `duration` is a positive number.
 */
 ...
 * the sound state has been scheduled (`spec/interaction-events.md:177-178`); the payload carries
 ...
 * profile-neutral generic catch-all (`spec/execution-model.md:1063`, via {@link PrimitivePayload} —
