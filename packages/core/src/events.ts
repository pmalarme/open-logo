...
/**
 * The registered kinds whose envelope **may carry** a `turtle_id`. `spec/execution-model.md:771` is
 ...
 * default applied when the caller omitted them (`spec/execution-model.md:908-946`'s worked
 ...
 * (`spec/execution-model.md:908-946`, e.g. `{name:"countdown", result:0}`). `result` is `null`
 ...
 * (`spec/execution-model.md:908-946`, e.g. `{value:0}`). Emitted only when a procedure actually
 ...
 * (`spec/execution-model.md:773-785`). Present in {@link TutorOutputPayload} only when
 * `command` is `"hint"`.
 */
 ...
 * (`spec/execution-model.md:836`) — so it is profile-neutral and the set of emitters is
 ...
 * "present only when the event is turtle-specific" (`spec/execution-model.md:771`): addressing
 ...
 * without a more specific event (`spec/execution-model.md:836`) — profile-neutral, not scoped to
 ...
 * closed (`spec/execution-model.md:822-827`, "One registered event kind"), the only sanctioned
 ...
 * catch-all (`spec/execution-model.md:836`): its `name` must stay an OPEN type so a future primitive
 ...
 * profile-neutral generic catch-all (`spec/execution-model.md:836`, via {@link PrimitivePayload} —
