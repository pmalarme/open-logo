...
/**
 * The registered kinds whose envelope **may carry** a `turtle_id`. `spec/execution-model.md:655` is
 ...
 * default applied when the caller omitted them (`spec/execution-model.md:792-830`'s worked
 ...
 * (`spec/execution-model.md:792-830`, e.g. `{name:"countdown", result:0}`). `result` is `null`
 * when the invocation is a command — it finished (or `stop`ped) without reaching `return`
 * (`spec/execution-model.md:374-380`) — rather than `0`/`false`/an empty list, which are
 ...
 * (`spec/execution-model.md:792-830`, e.g. `{value:0}`). Emitted only when a procedure actually
 ...
 * (`spec/execution-model.md:657-669`). Present in {@link TutorOutputPayload} only when
 * `command` is `"hint"`.
 */
 ...
 * (`spec/execution-model.md:720`) — so it is profile-neutral and the set of emitters is
 ...
 * "present only when the event is turtle-specific" (`spec/execution-model.md:655`): addressing
 ...
 * without a more specific event (`spec/execution-model.md:720`) — profile-neutral, not scoped to
 ...
 * closed (`spec/execution-model.md:706-711`, "One registered event kind"), the only sanctioned
 ...
 * catch-all (`spec/execution-model.md:720`): its `name` must stay an OPEN type so a future primitive
 ...
 * profile-neutral generic catch-all (`spec/execution-model.md:720`, via {@link PrimitivePayload} —
