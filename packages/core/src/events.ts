...
/**
 * The registered kinds whose envelope **may carry** a `turtle_id`. `spec/execution-model.md:942` is
 ...
 * default applied when the caller omitted them (`spec/execution-model.md:1079-1117`'s worked
 ...
 * (`spec/execution-model.md:1079-1117`, e.g. `{name:"countdown", result:0}`). `result` is `null`
 * when the invocation is a command — it finished (or `stop`ped) without reaching `return`
 * (`spec/execution-model.md:659-665`) — rather than `0`/`false`/an empty list, which are
 ...
 * (`spec/execution-model.md:1079-1117`, e.g. `{value:0}`). Emitted only when a procedure actually
 ...
 * (`spec/execution-model.md:944-956`). Present in {@link TutorOutputPayload} only when
 * `command` is `"hint"`.
 */
 ...
 * (`spec/execution-model.md:1007`) — so it is profile-neutral and the set of emitters is
 ...
 * `primitive`", `spec/interaction-events.md:153-154`; "wait emits a `primitive` event after the
 * pause completes … event registration forms emit `primitive` events after the handler is
 * registered", `spec/interaction-events.md:168-170`), but the type deliberately does not close over
 * them.
 */
 ...
 * "present only when the event is turtle-specific" (`spec/execution-model.md:942`): addressing
 ...
 * without a more specific event (`spec/execution-model.md:1007`) — profile-neutral, not scoped to
 ...
 * ("primitives without a more specific kind emit `primitive`", `spec/interaction-events.md:153-154`).
 * It is deliberately NOT a new registered `kind`: the registry's `kind` values are normative and
 * closed (`spec/execution-model.md:993-998`, "One registered event kind"), the only sanctioned
 ...
 * catch-all (`spec/execution-model.md:1007`): its `name` must stay an OPEN type so a future primitive
 ...
 * `spec/interaction-events.md:334-347`): the tempo, in beats per minute, that `set_tempo` set.
 * Durations elsewhere in the stream are carried in beats and interpreted at the current tempo
 * (`spec/interaction-events.md:342-343`). A positive number (`ol-range` otherwise);
 ...
 * `spec/interaction-events.md:349-366`): one pitched sound scheduled at the current tempo. `pitch`
 ...
 * its positive beat `duration` (`spec/interaction-events.md:368-382` — the melody list is
 ...
 * `spec/interaction-events.md:368-382`): the resolved melody, as an ordered list of pitch/duration
 ...
 * `spec/interaction-events.md:384-399`): one short, implementation-defined alert sound. It carries
 ...
 * `spec/interaction-events.md:401-416`): scheduled silence of `duration` beats at the current
 * tempo. `rest` emits a `sound` event "so replay tools can show the silent interval"
 * (`spec/interaction-events.md:410`). `duration` is a positive number.
 */
 ...
 * the sound state has been scheduled (`spec/interaction-events.md:168-169`); the payload carries
 ...
 * profile-neutral generic catch-all (`spec/execution-model.md:1007`, via {@link PrimitivePayload} —
