 ...
 * `primitive`", `spec/interaction-events.md:166-167`; "wait emits a `primitive` event after the
 * pause completes … event registration forms emit `primitive` events after the handler is
 * registered", `spec/interaction-events.md:181-183`), but the type deliberately does not close over
 * them.
 */
 ...
 * `spec/interaction-events.md:347-360`): the tempo, in beats per minute, that `set_tempo` set.
 * Durations elsewhere in the stream are carried in beats and interpreted at the current tempo
 * (`spec/interaction-events.md:355-356`). A positive number (`ol-range` otherwise);
 ...
 * `spec/interaction-events.md:362-379`): one pitched sound scheduled at the current tempo. `pitch`
 ...
 * its positive beat `duration` (`spec/interaction-events.md:381-395` — the melody list is
 ...
 * `spec/interaction-events.md:381-395`): the resolved melody, as an ordered list of pitch/duration
 ...
 * `spec/interaction-events.md:397-412`): one short, implementation-defined alert sound. It carries
 ...
 * `spec/interaction-events.md:414-429`): scheduled silence of `duration` beats at the current
 * tempo. `rest` emits a `sound` event "so replay tools can show the silent interval"
 * (`spec/interaction-events.md:423`). `duration` is a positive number.
 */
 ...
 * the sound state has been scheduled (`spec/interaction-events.md:181-182`); the payload carries
