...
- `lessons/level-5.ts` — the Level 5 lessons + graded exercises. "`define` names a reusable idea;
  ...
  procedure sets are its own automatically, and re-assigning an input never changes the
  caller's variable. The graded exercises ramp from a single-line change to the lesson's `polygon`
