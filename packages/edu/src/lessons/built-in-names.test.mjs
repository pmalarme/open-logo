...
//
// The maintainer ruling in `spec/grammar.md#keywords-primitives-and-built-in-names` is one sentence: **"A program may not declare a
// built-in name. A program may bind a value to any name."** `spec/grammar.md#keywords-primitives-and-built-in-names` names the four
// declaration slots (`define`, the heritage `to`, `struct`, and the first operand of `alias`), and
// `spec/grammar.md#keywords-primitives-and-built-in-names` frees every binding form. For the curriculum that has one consequence in
...
// (`spec/grammar.md#keywords-primitives-and-built-in-names`), so for a *procedure* declaration it is produced only by `check()`: a
...
// evaluates the rule of `spec/grammar.md#keywords-primitives-and-built-in-names` directly off the registries `@openlogo/parser`
// publishes — the keyword list under **every** profile (`spec/grammar.md#keywords-primitives-and-built-in-names` — profile words are
...
/**
 * Every primitive table the spec's built-in-name set draws on (`spec/grammar.md#keywords-primitives-and-built-in-names` — "every
...
/** Every Heritage alias spelling; `spec/grammar.md#keywords-primitives-and-built-in-names` makes an alias a built-in name too. */
 ...
 *
 * - **Profiles are never gated.** `spec/grammar.md#keywords-primitives-and-built-in-names` makes every profile's keywords and
 *   primitives built-in names whether or not that profile is claimed.
 * - **The name is folded first.** `spec/grammar.md#lexical-form-and-encoding` makes identifiers case-insensitive, so
 ...
 * it below is the tripwire that keeps this from being forgotten. `spec/grammar.md#keywords-primitives-and-built-in-names` names the
 ...
 * (`spec/grammar.md#lexical-form-and-encoding`) is not a declaration. Until the slot is reachable, the corpus gate covers
...
// `spec/grammar.md#keywords-primitives-and-built-in-names` — "**Nothing shadows.** `define count`, `define forward`, and `define fd`
...
// comment; folding case the way the host language does rather than the way `spec/grammar.md#lexical-form-and-encoding`
// does; and dropping an identifier's `?`/`!` suffix (`spec/grammar.md#ebnf-notation`) so a declaration is
// attributed to the wrong name — `empty` is explicitly not a built-in name (`spec/grammar.md#keywords-primitives-and-built-in-names`) while `empty?`
...
// The fourth declaration slot of `spec/grammar.md#keywords-primitives-and-built-in-names` is not modeled, and this is the tripwire
...
// AC2: `spec/grammar.md#keywords-primitives-and-built-in-names` protects `spec/educational-model.md#level-5--functions-and-procedures` ("Learners build `polygon`
// from `repeat`") by making the derived Geometry standard library OpenLogo *source* rather than
// primitives — while the renderer-backed overlays of `spec/educational-model.md#level-6--geometry-and-mathematics` stay built in.
...
// may declare one. `challenge` is the Tutor (AI) profile's (`spec/conformance.md#tutor-ai`) and
// reaches {@link builtInKind} through `tutorPrimitiveArity`; `spec/grammar.md#keywords-primitives-and-built-in-names` makes it owned
...
// The other half of `spec/grammar.md#keywords-primitives-and-built-in-names`: binding is completely free, so a lesson may name a
