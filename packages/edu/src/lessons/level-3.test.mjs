...
test("spec/educational-model.md's Level 3 lifetime bullet names both sides, and the runtime produces them", () => {
  ...
  //
  // What it enforces: the measured construct, outcome-to-side, and mutual exclusion of the two
  // outcomes. What it does NOT enforce is polarity in general. The negation check rejects three
  // spellings (`not`, `never`, `-n't`); English negation is open-class, so "fails to start
  // fresh", "no longer carries" or "rarely carries" keep every keyword in its correct clause and
  // still invert the meaning. Extending the denylist is a regress that widens the false-failure
  // surface on maintainer-owned prose without ever being complete, so this is deliberately a
  // tripwire for the commonest cases, not a proof. A red result here is a prompt to re-read the
  // sentence, not proof that the sentence is wrong.
