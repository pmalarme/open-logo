...
test("spec/educational-model.md's Level 3 lifetime bullet names both sides, and the runtime produces them", () => {
  ...
  // It remains blind to a paraphrase that keeps every keyword in place and changes the meaning
  // some other way; it binds outcome-to-side and polarity, not semantics in general.
  ...
  for (const clause of [insideClause, outerClause]) {
    assert.doesNotMatch(
      ...
      `a negated clause asserts the opposite of the measured behavior below: ${clause}`,
    );
  }

  // The construct the measured programs below actually exercise. Without this the bullet could be
  // re-aimed at another block form while the evidence kept running `repeat` (@ai-tutor, round 4).
  assert.match(
    bullet,
    /`repeat`/,
    `the lifetime bullet no longer names the construct these programs measure: ${bullet}`,
  );
