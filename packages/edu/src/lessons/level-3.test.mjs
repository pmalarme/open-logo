...
test("spec/educational-model.md's Level 3 lifetime bullet names both sides, and the runtime produces them", () => {
  ...
  // the for block") is in the same family. Extending the checks to chase these is a
  ...
  // check narrows by position rather than by a maintained list of construct names, which would
  // forfeit the closed form. A construct named WITHOUT backticks ("before the for block") is not
  // reachable this way and stays disclosed above rather than chased.
  const namedBlocks = [
    ...new Set(
      [...bullet.matchAll(/`[^`\r\n]+`(?=\s+(?:block|loop))/g)].map(
        (match) => match[0],
