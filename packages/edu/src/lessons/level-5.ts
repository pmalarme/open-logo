 ...
 * (`spec/execution-model.md:545-569`), and `local` — which declares a name private in the
 * current scope, shadowing anything of that name that was visible
 * (`spec/execution-model.md:501-506`) — is no longer taught here at all: now that a procedure's
 * own names are private without it, what is left is a narrower idea than this level's headline.
 *
