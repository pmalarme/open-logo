...
- A **diagnostic `message`** is compared **only when the fixture sets `"compareMessages": true`**
  (issue #1025). The default is identity — `code` + `params` (`spec/error-model.md:255-260`) — and
  `:262-264` positively permits a template author to reword prose, so most learner wording is
