...
description: >-
  How @turtle-engine consumes the normative trace/event stream (event kinds/types owned by
  @openlogo/core) and reduces it into a deterministic, render-agnostic turtle-state/frame contract in
  @openlogo/turtle — so studio, Canvas/SVG/PNG export, and a11y all agree. Use for turtle state, pen,
  heading, shapes, rendering.
...
2. **Model state** `{ x, y, heading, penDown, color, width, shape, visible }` (plus scene/background
   state, kept separate from per-turtle state) and reduce the runtime event stream into ordered,
   deterministic turtle frames — no randomness, no wall-clock.
...
6. **Publish the turtle-state/frame contract** (the reduced frames + render API) that `@openlogo/studio`
   consumes. The runtime event **kinds/types are owned by `@openlogo/core`**; you own the **reducer and
   frames**, not the event registry. Changes are serialized, owner-reviewed PRs.
