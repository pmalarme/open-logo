...
- **Cite the spec by section anchor** — `spec/<file>.md#a-heading`, not `:<line>`. An anchor does not
  move when text is inserted above it; a line number is only for a claim that genuinely needs line
  precision, and then it carries its anchor too. Both forms are accepted and migration is
  **incremental, never a mass edit** —
