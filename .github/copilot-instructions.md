...
**Agentic workflows** (`.github/workflows/*.md`, owned by `@devops`) are ordinary contributors under
these same rules — no special exception. The `.md` is the reviewable source, the paired
`*.lock.yml` is **generated by `gh-aw compile`, never hand-edited**, and a workflow's writes must
stay inside `gh-aw`'s `safe-outputs`. See
[`workflows.instructions.md`](instructions/workflows.instructions.md#agentic-workflows-githubworkflowsmd)
for the full operating rules and [ADR-0019](../docs/adr/0019-adopt-agentic-workflows.md) for the
rationale.
