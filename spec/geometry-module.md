...
The exterior-turn reasoning below uses `right` with a positive, unsigned exterior-angle magnitude throughout — `polygon`, `star`, and `arc` do not need a negative angle or the signed `turn` command to close a shape, since every exterior turn they compute is a positive quantity by construction. `right`, `left`, and `turn` accept signed angles as specified in [commands.md](commands.md); a geometry helper or learner program that wants a turn in the opposite sense from what is shown here MAY substitute a negated angle or `turn` freely, since `right :angle` and `left :angle` remain exactly `turn :angle` and `turn (0 - :angle)` respectively. Renderers animate every turn here, like any other `turn` event, through its signed `delta` rather than the shortest arc between headings, per [rendering.md](rendering.md#turtle-avatar-and-shapes).
