...
`export` is **not** a declaration slot. `export <identifier>` names a procedure the program has already defined, so it is a reference: `export if` fails as an unknown name rather than as a built-in-name collision, because no procedure named `if` can exist. The rest of the Modules profile is not specified in v0.1.0 (see [conformance.md](conformance.md#modules)).
...
Freeing the binding forms does not turn a keyword into an ordinary word everywhere else. A keyword remains structural in every position the grammar recognizes it: `:value = 1` binds a variable and `{ value: 1 }` names a key, but a bare `value` where an expression is expected is still the head of the heritage `value of … for key …` reader, not a name to look up. A keyword appearing in a position the grammar does not admit is a parse error as usual — it is never silently accepted as an ordinary name.
