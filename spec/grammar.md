...
Reserved words are structural tokens recognized by the reader. They may not be redefined as variables, procedures, primitives, or struct type constructors; such collisions raise `ol-reserved-word`. The restriction applies to **every form that introduces a name**: `define`/`to` procedures, `struct` type constructors, `local` declarations, procedure parameters, loop and comprehension binders, assignment — the place head of `:name = ...`, `set name to ...`, and the heritage `make "name" ...` — and, when the Localization profile is active, the **new name** an `alias` introduces, that is its `new_name` operand and never the `existing_name` it points at (see [localization.md](localization.md#alias-special-form)). It is not limited to the declaration forms, and the `:name` variable namespace is not an exception: assignment is the primary way to create a variable in OpenLogo and `local` is optional, so a restriction enforced only at `local` could be bypassed by omitting an optional keyword. Reserved words may be aliased by `alias` or localized keyword packs because aliasing adds reader-recognized spellings rather than redefining the underlying word.
