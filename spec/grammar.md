...
Reserved words are structural tokens recognized by the reader. They may not be redefined as variables, procedures, primitives, or struct type constructors; such collisions raise `ol-reserved-word`. The restriction applies to **every form that introduces a name**: `define`/`to` procedures, `struct` type constructors, `local` declarations, procedure parameters, loop and comprehension binders — including each name inside a destructuring pattern — assignment to a **plain** variable with no postfix (`:name = ...`, `set name to ...`, and the heritage `make "name" ...`), and, when the Localization profile is active, the **new name** an `alias` introduces, that is its `new_name` operand and never the `existing_name` it points at (see [localization.md](localization.md#alias-special-form)). A write *into* an existing value introduces no name and therefore never raises `ol-reserved-word`: `:people.repeat = 1` and `:nums[1] = 9` are unrestricted, because a postfix names a field or key, which is data. The rule is not limited to the declaration forms, and the `:name` variable namespace is not an exception: assignment is the primary way to create a variable in OpenLogo and `local` is optional, so a restriction enforced only at `local` could be bypassed by omitting an optional keyword. Reserved words may be aliased by `alias` or localized keyword packs because aliasing adds reader-recognized spellings rather than redefining the underlying word.
...
That namespace split decides which collisions the rule finds where. A form that **binds a variable** — assignment to a plain variable (`:name = ...`, `set name to ...`, `make "name" ...`), a loop or comprehension binder, a procedure parameter — collides with a **reserved word** only: a variable does not enter the callable namespace, so `:count = 1` is well-formed even though `count` is a Core primitive, while `:repeat = 1` is not. A form that **registers a name** — `define`/`to`, `struct`, `local`, `alias` — additionally requires the name to be fresh against primitives, existing procedures, and struct type constructors, so `local count` and `define count` both raise `ol-reserved-word` with `namespace: "primitive"`. **`local` is the deliberate exception in that split:** it binds a variable rather than entering the callable namespace, yet it requires the same freshness as the registration forms — so `local count` raises while `:count = 1` does not. `struct` **field** names introduce no name in either namespace, because they live in the per-type namespace above; a reserved word or a primitive name is therefore a legal field, and `struct point [ repeat y ]` is well-formed.
