...
Horizontal whitespace and indentation are insignificant except as token separators and in the tokenization of a leading `-` described below. A newline ends the current statement at the top level and inside a bracketed `[ ... ]` or long `... end` control body, **unless a continuation is pending**. Inside `[ ... ]` the newline is optional, because fixed arity also separates adjacent instructions. Immediately after a control or procedure header, a newline selects the long `... end` body form. Continuations operate at two levels. **Expression-level continuation** keeps one expression open across a newline; the following list is closed and exhaustive for the Core Language profile (optional profiles such as Heritage may define additional multi-token readers, for example the `value-of-reader`, whose incomplete positions also open continuations, documented in their respective sections): (1) **an unclosed delimiter** — the source is inside a parenthesized group `( ... )`, a list literal `[ ... ]`, or a dict literal `{ ... }` that has not yet reached its closing bracket, so newlines inside these constructs are insignificant; (2) **a trailing operator, operator keyword, or incomplete production** — the line ends with a token whose grammar production requires at least one more token: any infix operator (the tokens that appear between operands in the `or-expression`, `and-expression`, `comparison`, `additive`, or `multiplicative` productions — `or`, `and`, `==`, `!=`, `<`, `>`, `<=`, `>=`, `+`, `-`, `*`, `/`, `mod`), the unary prefix operator `not`, or the comparison keyword `is` and the multi-token predicates it begins (`is empty`, `is member of`, `is a`, `is [ strictly ] between`); once an `is-predicate` has begun, the entire predicate is a single expression and newlines within it are insignificant (for example, `5 is between 1` followed by a newline and `and 9` is one expression); (3) **a leading infix operator, comparison keyword, or field accessor** — the next line begins with an infix operator token (from the set in item 2), or with the comparison keyword `is` followed by a valid predicate form, so the previous line's final operand becomes the left operand of that operator or predicate, or the next line begins with `.` followed by an identifier (a field accessor on the previous line's result). **Statement-level continuation** keeps a control statement open after its bracket body has closed: (4) **`else` after a bracket-form `if`** — after the closing `]` of an `if-bracket-tail` (see the `if-statement` production), the parser looks ahead past any number of newlines, including blank lines, for an optional `else` keyword; if found, newlines between `else` and its required bracket body are likewise skipped, and the `else` clause is part of the same `if` statement. No other control form has an optional trailing clause after a bracket body, so `else` is the only statement-level continuation trigger. Arity never opens a continuation at either level: a command or reporter that has received fewer inputs than its signature requires does **not** reach across a newline to consume more; the statement terminates, and the arity checker reports `ol-not-enough-inputs`. Whether a leading `-` on a new line opens a continuation depends on tokenization: `- 5` (with a space) lexes as the infix subtraction operator followed by the literal `5`, which is a leading infix operator and therefore continues the previous statement; `-5` (without a space) lexes as the negative numeric literal `-5`, which is not an operator and therefore starts a new statement. This is a consequence of the lexical rule in the "Numbers" paragraph above: a `-` directly before a numeral with no left operand is part of the literal. Because `- 5` and `-5` produce different programs, horizontal whitespace at the start of a line can affect tokenization and therefore statement structure — an exception to the general insignificance rule. The style lint `ol-style-ambiguous-continuation` flags such lines to make the ambiguity visible (see [tooling.md](tooling.md)). Indentation preceding the leading token of a continuation line does not change which continuation trigger applies or the meaning of the program; only the identity of the leading token matters. Consecutive newlines form a single separator between statements; blank lines may appear freely between statements at the top level, inside `[ ... ]`, and inside a `... end` block. The `else` lookahead (item 4) is the one context where blank lines do not separate statements — they are skipped as part of the lookahead. The newline after the final statement of a file is optional.
