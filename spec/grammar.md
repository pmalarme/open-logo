...
**Matching a keyword.** Freeing the binding forms does not turn a keyword into an ordinary word everywhere else. A word in the keyword list is matched as that keyword's terminal wherever the grammar names it at the current position. It is never matched as `declared-callable-name`, `declared-type-name`, or `type-name`, and it is matched as `callable-name` only when the primitive matrix also gives that word a callable signature — in Core, `thing` alone. The positions that expect a plain `name`, an `identifier`, a `dict-key`, a `field-list` field, or a `key-term` do admit keywords, because those name data rather than structure: that is what makes `:value = 1`, `{ value: 1 }`, `local end`, and `for end from 1 to 3` legal. A keyword in a position none of those cover has no derivation and is a parse error, never a silently accepted name: `repeat key [ ]` does not read as a call to a procedure named `key`, and a bare `value` where an expression is expected is the head of the heritage `value of … for key …` reader or nothing at all.
