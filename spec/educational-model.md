...
- Parameters are variables such as `:sides` and `:size`; an input belongs to the procedure, so re-assigning one — giving it a new value — never changes the caller's variable.
- `return` hands a value back from a reporter.
- A command procedure may draw without returning a value.
- A procedure's variables are private automatically: `define … end` is a boundary, so its body sees exactly three kinds of variable — its own parameters, the bindings its body has already created, and names declared `global`. Procedure and command names are not hidden by it.
...
`local` is deliberately absent from this ramp. A procedure's variables are already private, so `local` is not what makes them so; it declares a name private in the current scope — a block, or a procedure body — shadowing anything of that name that was visible. That is a narrower idea than any level's headline. Learners nonetheless meet it in the standard-library source they are invited to read (see [geometry-module.md](geometry-module.md)), and a diagnostic may suggest it, so teaching material should be able to explain it as an aside without owing it a level.
