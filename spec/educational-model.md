...
`local` is deliberately absent from this ramp. A procedure's variables are already private, so `local` is not what makes them so; what it does beyond that is specified in [commands.md](commands.md#variables-and-output), and is a narrower idea than any level's headline. Learners nonetheless meet it in the standard-library source they are invited to read (see [geometry-module.md](geometry-module.md)), and a diagnostic may suggest it, so teaching material should be able to explain it as an aside without owing it a level.
