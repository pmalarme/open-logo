...
`challenge` belongs to the Tutor profile. Its canonical signature — a Command invoked as the bare word `challenge` with no inputs, producing tutor output rather than a program value — is specified normatively in [conformance.md](conformance.md#tutor-ai). It asks the learner to extend, vary, or explain their program. A challenge should be small enough to try now and open enough to invite discovery.
