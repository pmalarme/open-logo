...
The default avatar shape is an implementation-defined turtle-like shape whose nose points along the turtle heading. `set_shape` selects a named shape word. Implementations MUST support at least the default shape and SHOULD support the portable shape words below. Unknown shape words SHOULD raise the same style of learner-facing runtime diagnostic as other bad primitive inputs.

The **portable shape words** are `"turtle"`, `"triangle"`, `"arrow"`, and `"circle"`. Exact avatar geometry is a renderer presentation property. This is a portable set, not a closed value domain the way the `set_color` palette in [commands.md](commands.md#colors) is: an implementation MAY accept additional shape words. An implementation MUST publish a complete description of the shape words its `set_shape` accepts, including how it treats word case: it MAY do so by stating that it accepts the four portable words above and no others, together with how it matches word case; otherwise it MUST also list every portable word it omits and every additional shape word it accepts. OpenLogo v0.1 defines no portable mechanism for a program to declare a shape of its own; a future profile may define one.
