...
| Redefining a reserved word, primitive, existing procedure, existing type constructor, or existing alias | `ol-reserved-word` | A **reserved word** collides at every form that introduces a name — `define`, `to`, `struct`, `local`, `alias`, procedure parameters, loop and comprehension binders (each name in a destructuring pattern included), and assignment to a plain variable with no postfix (`:name = ...`, `set name to ...`, `make "name" ...`). A write into an existing value (`:p.field = ...`, `:nums[1] = ...`) introduces no name and MUST NOT raise `ol-reserved-word`. The primitive, procedure, and type-constructor categories apply only at the forms that **register** a name (`define`, `to`, `struct`, `local`, `alias`), so a checker MUST NOT reject `:count = 1`. See [grammar.md](grammar.md#reserved-words-and-namespaces). |
