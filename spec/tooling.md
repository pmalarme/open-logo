...
| `repcount` outside any lexically enclosing `repeat` | `ol-repcount-outside-repeat` | Decide enclosure lexically: a procedure body is never inside its caller's `repeat`, so `repcount` there is an error even when a `repeat` is running ([execution-model.md](execution-model.md#repcount-is-lexical)). |
