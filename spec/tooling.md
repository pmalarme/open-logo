...
|---|---|
| `keyword` | Structural words the reader recognizes as grammar, in the position they occupy. Three groups qualify: structural [reserved words](#reserved-words-for-tooling) other than the word-spelled operators `and`, `or`, `not`, and `mod`, which are `operator` below; the contextual words `empty`, `member`, `of`, and `a` in the structural positions described under that heading, though none of the four is a reserved word; and profile block-heads while their profile is active, this class following whether a word *works*, never whether a program may declare it. [Disambiguating identifiers](#disambiguating-identifiers) governs when symbol discovery gives a token a more specific class instead. This class therefore **overlaps** the reserved list without being derived from it — they are different sets on purpose ([grammar.md](grammar.md#keywords-primitives-and-built-in-names)), and reserved-list membership never by itself decides how a token is painted. |
