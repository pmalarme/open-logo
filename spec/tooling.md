...
| Undefined variable read | `ol-undefined-var` | Point at the `:variable` token or place head that reads an unbound value. Use it for every failed read the sealed procedure boundary is not the cause of — a name no scope binds at all, a read before the statement that creates the binding in the same scope, and a block's read of an enclosing binding created later, among others. When a procedure body reads a name that a scope enclosing it binds and the sealed procedure boundary is what hides it, report `ol-var-not-visible` instead. |
| A procedure reads a name the sealed procedure boundary hides | `ol-var-not-visible` | Visibility is the sealed lexical model of [execution-model.md](execution-model.md#what-a-scope-can-see): inside a procedure body only its parameters, the names its body has already bound, and `global` names are visible, so a procedure reading a plain non-`global` top-level name is this diagnostic, not an outer reference. Point at the READ, carry `name` and the enclosing `procedure`, and suggest `global {name} = …`. |
...
| `repcount` outside any lexically enclosing `repeat` | `ol-repcount-outside-repeat` | Decide enclosure lexically: a procedure body is never inside its caller's `repeat`, and a handler block is never inside a `repeat` written outside it, so `repcount` in either is an error even when a `repeat` is running. A `repeat` written inside the handler block encloses normally ([execution-model.md](execution-model.md#repcount-is-lexical)). |
