...
|---|---|
| `keyword` | The words the reader itself gives a grammatical role to. Enumerated here in this class's own terms: the statement, block, and control-transfer heads and their slot words — `define`, `to`, `end`, `return`, `output`, `op`, `stop`, `throw`, `set`, `make`, `local`, `if`, `else`, `while`, `repeat`, `for`, `forever`, `in`, `from`, `at`, `by`, `key`, `value`, `add`, `remove`, `insert`, `clear`, `map`, `filter`, `reduce`, `struct`, `alias`, `import`, `export`; the worded predicates `is`, `between`, and `strictly`; the boolean literals `true` and `false`; `thing`, which takes this class even though the C3 matrix also gives it a callable reporter form; the contextual words `empty`, `member`, `of`, and `a` in the structural positions described under [Reserved words](#reserved-words-for-tooling), none of which is a reserved word; and, while their profile is active, the profile block-heads together with the Sprites mode-switch command `tell`, which takes no block. The word-spelled operators `and`, `or`, `not`, and `mod` are **not** in this class — they are `operator` below. The lexical first pass paints the words named above — other than the four contextual ones, which take this class only in the structural positions just described and are ordinary names elsewhere, so `local empty` is not a keyword — wherever they appear, including the positions where the grammar admits one as an ordinary name (`local end`, `for end from 1 to 3`, `export end`, `:p.end`). A bare key in `{ key: value }` or `:dict[key]` is `dict-key` on grammatical grounds alone and needs no symbol discovery; [Disambiguating identifiers](#disambiguating-identifiers) is what demotes a token to `procedure-name`, `type-name`, or `field-name` once symbol discovery resolves it. This class is **not derived from** the reserved list — it omits four reserved words, adds four non-reserved ones, and adds the profile words — so reserved-list membership never decides how a token is painted ([grammar.md](grammar.md#keywords-primitives-and-built-in-names)). |
| `primitive` | Built-in commands, reporters, and aliases from the C3 primitive matrix, including full names such as `forward`, one-word aliases such as `setcolor`, short aliases such as `fd`, heritage command aliases such as `pr`, and profile primitives when enabled. Where another row of this table names a word explicitly — `thing`, `true`, and `false` under `keyword`, the word-spelled operators under `operator`, and the profile words while their profile is active — that assignment wins, so a word never carries two classes. Structural special-form heads are `keyword` unless they are being documented as callable entries. |
