...
| Registered primitive with no evaluation behind it | `ol-not-implemented` | A name this specification defines and the implementation registered, but cannot run. Report it here **only** when the checker knows before running that no evaluation exists; otherwise it is the run's to report ([error-model.md](error-model.md)). An implementation MUST NOT reach for `ol-unknown-command` instead — including by withholding the name from the visible vocabulary so the call reads as unknown — because the name is known and the two codes exist to keep *you mistyped a name* apart from *we have not built this yet*. |
