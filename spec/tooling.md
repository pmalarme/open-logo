...
| Undefined variable read | `ol-undefined-var` | Point at the `:variable` token or place head that reads an unbound value. Use it for every failed read except the sealed-boundary one: a name no scope binds at all, and a block's read of an enclosing binding created later in the enclosing scope. When a procedure body reads a name that a scope enclosing it binds and the sealed procedure boundary is what hides it, report `ol-var-not-visible` instead. |
