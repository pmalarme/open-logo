...
| Undefined variable read | `ol-undefined-var` | Point at the `:variable` token or place head that reads an unbound value. Use it when the name is bound nowhere the read can reach; when a procedure body reads a name the sealed boundary hides, report `ol-var-not-visible` instead. |
...
| `repcount` outside any lexically enclosing `repeat` | `ol-repcount-outside-repeat` | Decide enclosure lexically: a procedure body is never inside its caller's `repeat`, and a handler block's invocation is never on a turn of any loop, so `repcount` in either is an error even when a `repeat` is running ([execution-model.md](execution-model.md#repcount-is-lexical)). |
