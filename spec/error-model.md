...
| `ol-undefined-var` | runtime | `name` | Reading `:name` or `thing` found no binding of that name the read can reach: either no scope binds the name at all, or a block read an enclosing binding created later in the enclosing scope. When a procedure body reads a name that a scope enclosing it binds but the sealed procedure boundary hides, the code is `ol-var-not-visible` instead. |
