...
| `ol-reserved-word` | semantic | `name`, `namespace` | A program attempted to redefine or collide with a reserved keyword, primitive, existing procedure, type constructor, or alias new name where freshness is required. A **reserved word** must be fresh at **every form that introduces a name** — not only at the declaration forms, so assignment and binders are covered too — while the primitive, procedure, and type-constructor categories are required only at the forms that **register** a name (`define`/`to`, `struct`, `local`, `alias`); `:count = 1` is therefore well-formed even though `count` is a primitive. [grammar.md](grammar.md#reserved-words-and-namespaces) owns the normative list of forms and that binding-versus-registration boundary. |
