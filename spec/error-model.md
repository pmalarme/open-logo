...
| `ol-undefined-var` | runtime | `name` | Reading `:name` or `thing` found no binding of that name the read can reach at all. When a procedure body reads a name that a scope enclosing it binds but the sealed procedure boundary hides, the code is `ol-var-not-visible` instead. |
...
| `ol-var-not-visible` | semantic | `name`, `procedure` | A procedure body read a name that a scope enclosing the read binds, but which the sealed procedure boundary hides: it is not one of the procedure's parameters, not a binding its body has already made, and not a `global` ([execution-model.md](execution-model.md#what-a-scope-can-see)). The two codes partition the failed read by whether such a binding exists, and the test is **lexical, not temporal** — a name an enclosing scope binds anywhere in the declaring document takes this code whether or not the binding has been created when the read executes, which is what keeps it decidable at the `semantic` stage. Because the name exists and the boundary is the reason, the message MUST name the boundary — `:{name} is not defined inside {procedure}` — and the suggestion MUST name the fix, `global {name} = …`. A generic "undefined variable" message is not sufficient for this code. |
