...
| `ol-reserved-word` | semantic | `name`, `namespace` | A program attempted to redefine or collide with a reserved keyword, primitive, existing procedure, type constructor, or alias target where freshness is required. Freshness is required at **every form that introduces a name** — not only at the declaration forms — so assignment and binders are covered too; [grammar.md](grammar.md#reserved-words-and-namespaces) owns the normative list of forms. |
