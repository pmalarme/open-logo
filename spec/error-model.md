...
| `ol-unknown-type` | semantic | `name` | A type name in a **type position** — the type word of `is a` / `is_a?`, or a struct field's declared type — is not a known built-in type or declared struct. An unknown name in **callable position**, such as a constructor or command call, raises `ol-unknown-command` instead. |
