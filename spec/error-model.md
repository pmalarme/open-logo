...
| `ol-range` | runtime | `operation`, `index` or `value`, optional `length` | A number is outside the allowed range, including a 1-based list index out of range, an empty `first` or `last`, `insert` outside valid positions, `pick` from an empty list, a `repeat` count that is negative or not a whole number, or a destructuring pattern length mismatch. |
...
| `ol-return-in-comprehension` | semantic | `keyword`, `form` | `return`, `output`, or `op` appears inside a `map`, `filter`, or `reduce` body. Comprehensions report by their last expression instead. |
