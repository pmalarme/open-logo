...
| `ol-undefined-var` | runtime | `name` | Reading `:name` or `thing` found no value bound to that name the read can reach. This is every failed read the sealed procedure boundary is **not** the cause of — a name no scope binds at all, a read before the statement that creates the binding in the same scope, a read of a name a bare `local` has declared but nothing has yet assigned, and a block's read of an enclosing binding created later in the enclosing scope, among others. When a procedure body reads a name that a scope enclosing it binds but the sealed procedure boundary hides, the code is `ol-var-not-visible` instead. |
...
| `ol-var-not-visible` | semantic | `name`, `procedure` | A procedure body read a name that a scope enclosing the read binds, but which the sealed procedure boundary hides: it is not one of the procedure's parameters, not a binding its body has already made, and not a `global` ([execution-model.md](execution-model.md#what-a-scope-can-see)). The test is **lexical, not temporal** — a name an enclosing scope binds anywhere in the declaring document takes this code whether or not the binding has been created when the read executes, which is what keeps it decidable at the `semantic` stage. Every other failed read is `ol-undefined-var`. Because the name exists and the boundary is the reason, the message MUST name the boundary — `:{name} is not defined inside {procedure}` — and the suggestion MUST name the fix, `global {name} = …`. A generic "undefined variable" message is not sufficient for this code. |
