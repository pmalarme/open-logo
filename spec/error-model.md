...
| `ol-undefined-var` | runtime | `name`, optional `procedure` | Reading `:name` or `thing` found no visible binding. When the read is inside a procedure body and the name is one the sealed procedure boundary hides — it exists in an enclosing scope but is neither a parameter, nor a binding the body already made, nor a `global` ([execution-model.md](execution-model.md#what-a-scope-can-see)) — `procedure` names the enclosing procedure so the message can say `{name} is not defined inside {procedure}` and offer `global {name} = …` as the fix. |
