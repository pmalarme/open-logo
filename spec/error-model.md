...
| `ol-var-not-visible` | semantic | `name`, `procedure` | A procedure body read a name the sealed procedure boundary hides: the name is bound in an enclosing scope, but it is not one of the procedure's parameters, not a binding its body has already made, and not a `global` ([execution-model.md](execution-model.md#what-a-scope-can-see)). Distinct from `ol-undefined-var`, which says the name is bound nowhere the read can reach; here the name exists and the boundary is the reason, so the message MUST name the boundary — `{name} is not defined inside {procedure}` — and the suggestion MUST name the fix, `global {name} = …`. A generic "undefined variable" message is not sufficient for this code. |
