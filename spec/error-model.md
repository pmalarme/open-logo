...
| `ol-range` | runtime | `operation`, `index` or `value`, optional `length` | A number is outside the allowed range, including a 1-based list index out of range, an empty `first` or `last`, `insert` outside valid positions, `pick` from an empty list, a negative `repeat` count, or a destructuring pattern length mismatch. |
