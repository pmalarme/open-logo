...
| `ol-undefined-var` | runtime | `name` | Reading `:name` or `thing` found no binding of that name the read can reach. This is every failed read the sealed procedure boundary is **not** the cause of — a name no scope binds at all, a read before the statement that creates the binding in the same scope, and a block's read of an enclosing binding created later in the enclosing scope, among others. When a procedure body reads a name that a scope enclosing it binds but the sealed procedure boundary hides, the code is `ol-var-not-visible` instead. |
