 ...
 *
 * ## The line form is ratcheted — it may fall, never rise
 *
 * Recording the anchor convention put it in force as **guidance**; nothing stopped the next change
 * adding twenty fresh line-form citations, and among thousands of existing ones nobody would see
 * them (issue #1183). So the count is pinned: {@link lineFormCount} totals every counter that names
 * a line — explicit, comma-appended, bare, and the line fragment, which is exactly the set the
 * printed coverage statement already groups as "still drift whenever the spec is edited above
 * them" — and it is compared against the number committed in {@link BASELINE_PATH}.
 *
 * **Both directions fail.** Above the baseline is the defect the ratchet exists to catch. Below it
 * is a stale high-water mark, which would silently re-admit as many new line citations as the last
 * conversion removed — a ratchet that tolerates one has stopped ratcheting — so the run says to
 * lower the number in the same change, and how.
 *
 * **The baseline is generated by this gate, never hand-written** (`--write-baseline`). Two careful
 * manual counts of this corpus have already disagreed with each other, with nothing in the
 * repository able to adjudicate between them; that disagreement is part of why saga #1180 exists.
 *
...
/** Location of the committed line-form high-water mark the ratchet asserts against (issue #1183). */
export const BASELINE_PATH = join("scripts", "spec-citations-baseline.json");

/** The baseline file's own one-line account of what it is and how to regenerate it. */
export const BASELINE_NOTE =
  "Line-form spec citations (issue #1183): this count may fall, never rise. " +
  "Regenerate with `node scripts/check-spec-citations.mjs --write-baseline`.";
...
/**
 * How many of `counts` name a **line** — the quantity the ratchet pins (issue #1183).
 *
...
/**
 * The line-form high-water mark recorded in the baseline file.
 *
 * It **throws** rather than reporting a finding when the file declares no usable number, and the
 * asymmetry with the exceptions manifest is deliberate: a missing or malformed baseline is a broken
...
export function loadBaseline(baselinePath = BASELINE_PATH) {
  ...
  if (!Number.isInteger(count) || count < 0) {
    throw new Error(
      `${toPosixPath(baselinePath)} declares no non-negative whole number of "lineFormCitations", so the ` +
        "line form would not be ratcheted at all — regenerate it with " +
...
/**
 * Write `count` to the baseline file, with the note that says how it was produced.
 *
 * This is the only supported way to set the number. Hand-writing it is what {@link BASELINE_NOTE}
 * exists to prevent: two careful manual counts of this corpus have already disagreed, and a baseline
 * copied out of an issue is stale before it is committed, because prose *about* citations is itself
 * scanned.
 */
export function writeBaselineFile(baselinePath, count) {
  const document = { _note: BASELINE_NOTE, lineFormCitations: count };
...
export function runSpecCitationsGate({
  ...
  if (writeBaseline) {
    ...
    fail(
      `line-form spec citations fell to ${lineForm} against a baseline of ${baseline} (${lineForm - baseline}) — ` +
        "which is the direction this ratchet wants, but the recorded number is now a stale high-water mark that " +
        `would let ${baseline - lineForm} new line citation(s) back in unseen, so it no longer ratchets. Lower it ` +
        `in this same change: run ${regenerate} and commit ${toPosixPath(baselineFile)}.`,
    );
  }
...
/**
 * Parse CLI arguments: `--root=<path>` (repeatable), `--spec-dir=<token>`, `--spec-root=<path>`,
 * `--exceptions=<path>` and `--baseline=<path>` override the defaults, which is how the subprocess
 * regression tests point the CLI at isolated temp fixtures instead of the real corpus.
 * `--write-baseline` records the live line-form count as the new baseline instead of checking it —
 * the only supported way to set that number (issue #1183).
 */
