 ...
 *
 * `baseline: null` is explicit because the ratchet no longer infers a skip from `roots`: the
 * committed mark describes the repository, not a fixture tree, so a fixture run says so rather than
 * being quietly exempted (issue #1183). A test that wants the ratchet overrides it.
 */
...
test("baselineFor defaults to the committed mark and never infers a skip", () => {
  const path = join(TEMP_DIR, "baseline.json");
  writeBaselineFile(path, 7);
  assert.equal(baselineFor(path), 7);
  // The rule that matters: with no path named, the mark is the committed one — it is NOT inferred
  // away. Reading `roots` as "fixture tree, skip the ratchet" left `--root=.`, an ordinary way to
  // scan the repository, green with nothing ratcheted while the report still printed a number.
  const committed = baselineFor(undefined);
...
test("--write-baseline generates the number, and the tree it was written from then passes", () => {
  ...
  // A write run is a third state, and must read as neither "checked" nor "unchecked".
  assert.match(written.lines.join("\n"), /LINE-FORM 1 — just written to/);
  assert.match(written.lines.join("\n"), /REWROTE/);
  ...
  const checked = runOverTemp({}, { baselinePath, baseline: undefined });
  assert.equal(checked.ok, true);
  assert.match(checked.lines.join("\n"), /LINE-FORM 1 of 1 baseline, 0 failed/);
  // A run given no baseline still prints the count — like UNRESOLVED — but says outright that it
  // checked nothing, so a skip can never read as a pass. The suffix is the only tell, and the
  // footnote repeats it.
  const unratcheted = runOverTemp().lines.join("\n");
  assert.match(
    unratcheted,
    /LINE-FORM 1 \(NOT ratcheted — this scan was given no baseline\), 0 failed/,
  );
  assert.match(unratcheted, /REPORTED AND NOT CHECKED/);
  assert.match(
    checked.lines.join("\n"),
    /It is held to the baseline committed/,
  );
  // The regeneration command names the file that actually failed, not the default one.
  assert.match(
    runOverTemp({}, { baselinePath, baseline: 9 }).lines.join("\n"),
    /--write-baseline --baseline=[^`]*baseline\.json`/,
  );
});
...
test("MUTATION: a line-form citation added without converting one fails, naming the excess", () => {
  ...
  // The message has to say outright that the gate cannot tell an authored citation from an
  // inherited one — it counts the tree, not the change — and then name a runnable diff that narrows
  // the answer to the files this change touched, instead of leaving the reader to guess.
  assert.match(
    report,
    /cannot tell you whether you wrote that citation or inherited it/,
  );
  assert.match(report, /counts the tree rather than the change/);
  assert.match(
    report,
    /git diff <the branch this change targets>\.\.\.HEAD -G"contract\/\[\^ \]\*\\\.md" --stat/,
  );
  assert.match(report, /is a reviewable act rather than a fix/);
 ...
 *
 * It always names a baseline, because the CLI no longer exempts a rooted scan: `--root=.` is an
 * ordinary way to scan the repository, so inferring "fixture tree, skip the ratchet" from `--root`
 * left a full-repository run green with nothing ratcheted (issue #1183). Each caller writes the mark
 * its own fixture tree deserves.
 */
...
/** Write a baseline of `count` for the temp tree and return the `--baseline=` argument naming it. */
function cliBaseline(count) {
  const path = join(TEMP_DIR, "cli-baseline.json");
  writeBaselineFile(path, count);
...
test("the CLI exits 0 and prints the report when every citation resolves", () => {
  ...
  const { status, output } = runCli(exceptionsPath, [cliBaseline(1)]);
...
test("the CLI does NOT exempt a rooted scan — --root=. would otherwise ratchet nothing", () => {
  // The bypass this closes: `--root` used to mean "fixture tree, skip the ratchet", so
  // `node scripts/check-spec-citations.mjs --root=.` scanned the whole repository, exited 0, and
  // printed a LINE-FORM number that had been compared against nothing. A rooted run with no
  // --baseline is now held to the committed mark, which for a one-citation fixture tree is a loud
  // failure rather than a silent pass. The assertion is structural: it must not pin the corpus
  // total, or an unrelated change to the corpus would break it.
  writeGrammar();
  write("site.ts", "// contract/grammar.md:6 defines the selector.\n");
  const exceptionsPath = join(TEMP_DIR, "exceptions.json");
  writeFileSync(exceptionsPath, "{}", "utf8");
  const { status, output } = runCli(exceptionsPath);
  assert.equal(status, 1, "a skipped ratchet must never read as a pass");
  assert.match(output, /fell to 1 against a baseline of \d+/);
  assert.doesNotMatch(output, /NOT ratcheted/);
});
